...
**Executable tip now (updated 2026-08-05):** **F-001 undeferred; C-027 Locked.** Track A (animal life) opened and **§4.66** Herbivore population/trait fields shipped its machine half the same day — first `populations` Process registered ([BUILD_GUIDE](docs/BUILD_GUIDE.md), [CLOUD_AGENT_PIPELINE.md](docs/CLOUD_AGENT_PIPELINE.md) §3); D-007 clip verdict outstanding ([ask](docs/playtests/A1-herbivore.md)), gates any *other* new-Process slice, does not gate Track A's own tip, now **§4.67** Seed disperser (extends A1's Process). Track V's **§4.62** composite runner-up guild remains open in parallel. Everything else in this paragraph's earlier form still holds: Track R, Track T, and the Living wave (through L5) stay Done; remaining owner-judged taste (**C-014**, **C-021**, **C-022**, **C-026**, **C-028**) and **L8** (**C-024**/**C-025**) stay off tip. Mesopredator/apex-predator roles, ecosystem-engineer write-back, and adaptive radiation (**C-029**, Open) stay off Track A specifically — see [C-027-framing.md](docs/candidates/C-027-framing.md) §4.6/§5 and [C-029-framing.md](docs/candidates/C-029-framing.md).
...
- **Succession.** Merge-gated multi-agent pipeline: [docs/CLOUD_AGENT_PIPELINE.md](docs/CLOUD_AGENT_PIPELINE.md). One BUILD_GUIDE slice per cloud run. Track R, Track T, and the Living wave are Done. Two machine slices are open in parallel: **Track V §4.62** composite runner-up guild, and **Track A §4.67** Seed disperser fields (extends A1's `populations` Process — **§4.66** Herbivore shipped its machine half 2026-08-05, D-007 clip verdict outstanding, [ask](docs/playtests/A1-herbivore.md), gates any *other* new-Process slice but not this one). Claim one. Foxel silhouette/variant design for Track A is a separate low-cost lane, not a cloud-succession track: [docs/animal-design/PROTOCOL.md](docs/animal-design/PROTOCOL.md). If both tracks are taken and no owner decision has landed on the remaining backlog (C-024/C-025/C-026/C-028 taste, or the C-029 Lock for adaptive radiation), write a blocked note naming that backlog and stop; do not start mesopredator/apex-predator/ecosystem-engineer/adaptive-radiation work off your own initiative — those stay gated per [C-027-framing.md](docs/candidates/C-027-framing.md) §4.6/§5. Rebase onto `main`; leave tip accurate for the next merge-triggered agent.
