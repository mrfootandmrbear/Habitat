...
**Executable tip now:** two parallel tracks. Track R tips **§4.48** habitat/dispersal determinism hygiene. Track V (new, 2026-08-03) tips **§4.60** plant rendering — per-guild silhouette geometry, replacing `OccupantMesh`'s one shared cone with a distinct shape per vegetation guild ([plant rendering review](docs/reviews/2026-08-03-plant-rendering-review.md)); **§4.61** clustering and **§4.62** composite runner-up guild queued behind it; **§4.63** distance LOD deferred until those three are shipped and profiled. Track T (terrain tools) is parked: **§4.59** duplicator stamp shipped, completing the C-028 structural "keep" kit ([duplicator-stamp-composition.md](docs/slices/duplicator-stamp-composition.md)); no further machine slice, only owner taste. **§4.57** molds shipped; **§4.47** guild cover & light-competition correctness shipped ([guild-cover-light-composition.md](docs/slices/guild-cover-light-composition.md)) — product-complement cover `1 − Π(1 − cᵢ)`, Beer–Lambert inverse LAI `−ln(1 − cover)/k` with transmitted `I₀(1 − cover)` reaching 0 at full cover, growth reads open-fraction absorbed light once (no `(1 − cover)²` double-count); 22 baselines refreshed, no T-001 golden hash moved. **§4.58** Simple/Full chrome shipped; **§4.46** HSI curves shipped. Living-wave remainder blocked: **L5** on **C-023**; **L8** on **C-024** + **C-025**.
...
- **Succession.** Merge-gated multi-agent pipeline: [docs/CLOUD_AGENT_PIPELINE.md](docs/CLOUD_AGENT_PIPELINE.md). One BUILD_GUIDE slice per cloud run; claim Track R (§4.48) or Track V (§4.60) — Track T is parked, no further machine slice; rebase onto `main`; leave tip accurate for the next merge-triggered agent.
