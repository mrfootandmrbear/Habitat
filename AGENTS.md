...
**Executable tip now:** Track R tips **§4.48** habitat/dispersal determinism hygiene — the sole remaining executable queue. Track T (terrain tools) is parked: **§4.59** duplicator stamp shipped, completing the C-028 structural "keep" kit ([duplicator-stamp-composition.md](docs/slices/duplicator-stamp-composition.md)); no further machine slice, only owner taste. **§4.57** molds shipped; **§4.47** guild cover & light-competition correctness shipped ([guild-cover-light-composition.md](docs/slices/guild-cover-light-composition.md)) — product-complement cover `1 − Π(1 − cᵢ)`, Beer–Lambert inverse LAI `−ln(1 − cover)/k` with transmitted `I₀(1 − cover)` reaching 0 at full cover, growth reads open-fraction absorbed light once (no `(1 − cover)²` double-count); 22 baselines refreshed, no T-001 golden hash moved. **§4.58** Simple/Full chrome shipped; **§4.46** HSI curves shipped. Living-wave remainder blocked: **L5** on **C-023**; **L8** on **C-024** + **C-025**. **§4.60 weather presentation shipped** ([weather-presentation-composition.md](docs/slices/weather-presentation-composition.md)) — owner ask: rain/snow didn't read as coming from clouds, rain and snow shared one particle model, the snow ground-hold was a flat sheet, and the scene haze was inert. Named **G6–G9** in [C-020-dossier.md](docs/candidates/C-020-dossier.md), same pattern as G1–G5: `releasingCloudCount` maps regime intensity onto how many of `CloudMesh`'s bodies release at once, `RainCueMesh` spawns precip under their footprints; rain/snow fall kinematics diverge per phase; a terrain-weighted patchy mask (elevation + slope + noise) replaces the flat ground-cover sheet; scene fog now tracks storm veil + cloud cover instead of sitting at a fixed distance. Presentation-only (T-006), no new Process, C-020's Lock and its no-cell-targeting / no-SWE bans untouched.
