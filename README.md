...
**Next.** Three parallel executable tracks. Track R: **§4.48** habitat/dispersal determinism hygiene Done ([BUILD_GUIDE §4.48](docs/BUILD_GUIDE.md) — **§4.47** guild cover & light-competition correctness Done). Track V: plant rendering — **§4.60**/**§4.61** shipped, **§4.62** composite runner-up guild tip, **§4.63** deferred pending profiling ([plant rendering review](docs/reviews/2026-08-03-plant-rendering-review.md)). **Track A (animal life), opened 2026-08-05:** owner undeferred **F-001** and Locked **C-027** — **§4.66** Herbivore population/trait fields shipped its machine half the same day (first `populations` Process; D-007 clip verdict outstanding, [ask](docs/playtests/A1-herbivore.md)), tip is now **§4.67** Seed disperser (extends A1's Process, not blocked by the outstanding clip); Foxel silhouette/variant design for it is a separate low-cost lane, [docs/animal-design/PROTOCOL.md](docs/animal-design/PROTOCOL.md). Mesopredator/apex-predator roles, ecosystem-engineer write-back, and adaptive radiation (**C-029**, Open) stay off Track A. Living wave Done through **L5**; **L8** stays blocked. Track T (terrain tools) parked — no further machine slice, only owner taste. Owner Lock backlog (**C-014** hearable, **C-021**/**C-022**/**C-028** taste, **C-010** later, **C-029** adaptive radiation) runs in parallel.

**A second, parallel queue.** **§4.44** / **§4.45** / **§4.46** / **§4.47** / **§4.49–§4.53** shipped; **§4.48** remains queued.

**A third, parallel queue.** Plant rendering (**§4.60–§4.63**) — see Track V above.
