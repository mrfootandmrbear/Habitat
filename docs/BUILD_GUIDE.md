...
**Next (executable tip):** **Track V §4.62** composite runner-up guild and **Track A §4.66** Herbivore population/trait fields are the two open executable machine slices, running in parallel on disjoint tracks. **§4.60** per-guild silhouettes, **§4.61** per-cell clustering, **§4.64** occupant sway direction correctness, and **§4.65** weather presentation (C-020 G6–G9) are all shipped, as are Track R **§4.48** and **§4.40 L5** (**C-023 Locked**). **§4.63** distance LOD stays queued behind §4.62, gated on profiling. **§4.67** Seed disperser is Track A's next-but-one behind §4.66; **§4.68** adaptive radiation is a stub gated behind owner Lock on **C-029**. Track T parked on owner taste. **L8** remains blocked on **C-024** / **C-025**. Keep nutrients / SWE off the tip; animal competition (mesopredator/apex) and ecosystem-engineer roles stay off Track A per C-027-framing.md §4.6/§5.
...
### 4.66 Slice A1 — Herbivore population/trait fields *(queued — Track A tip, F-001 undeferred / C-027 Locked 2026-08-05)*
...
- [ ] New field family `pop.herbivore.trait.limbLength` / `.insulation` / `.webbing` (trait-mean) and `pop.herbivore.swap.webbing` (hysteretic latch state, C-027 §3.2/§3.4) — same band/legacy status as density
- [ ] Carrying capacity derived each annual step from habitat/resources/competition/predation — **never a stored constant** (**ES-006**); mesopredator/apex predation terms are honestly absent (those roles don't exist yet, C-027 §4.6.1), not a hidden `K`
- [ ] Trait update law (C-027 §3.3): `traitMean += traitRate · (pressureOptimum(habitatState) − traitMean) · dt`, `traitRate` derived from `pop.herbivore.stage[k]` turnover — never a hand-tuned constant; mismatch (`|pressureOptimum − traitMean|`) enters the mortality/capacity term so adaptation costs something (E-006/E-009); `traitMean` clamped to a per-species envelope from preserve data, population declines rather than exceeding it
- [ ] Plasticity/adaptation split (C-027 §3.3): `insulation` is reversible on the `seasonal` band; `webbing` is slow/hysteretic on the `annual` band — two derived rates on two bands, not one law
- [ ] Three pressure mappings, each with a real-world referent (**C-011**): `limbLength` ← terrain slope factor (continuous, bone-scale); `insulation` ← `factorTemperature` (`temperatureComposition.ts`, continuous, bone-scale, seasonal); `webbing` ← `f_inundation` (**NS-008**, discrete rung-swap at a two-value hysteretic threshold, annual)
- [ ] Grazing write-back (C-027 §4.6.3): herbivore density contributes a mortality-rate term to the grazed guild's `veg.biomass.*` via `Process.contributes` + the §11.2 delta inbox — `vegetation` stays the sole owner; zero herbivore density is a no-op against today's baselines (same zero-is-a-no-op discipline **L2** proved)
...
- [ ] Render: one new `InstancedMesh` extending `OccupantMesh`'s pattern; instance count = `density × cellAreaKm2` capped at a render budget — literal readout, never tuned (C-027 §2); per-instance deterministic seed from `(cellIndex, instanceIndex[, tick])`, no persisted identity across frames (**T-001**, **T-006**)
- [ ] Base mesh sourced from the animal-design lane ([docs/animal-design/PROTOCOL.md](animal-design/PROTOCOL.md)) — at minimum one accepted morph card per trait (limbLength, insulation, webbing) naming rungs and a real-world referent before this slice builds its Foxel `.fxl`/`.glb` assets; `limbLength`/`insulation` as skeleton bone-scale, `webbing` as a rung swap from a pre-baked `.glb` ladder
- [ ] Tier-M: conservation/bounds on density and trait-mean fields; determinism (identical seed/state → identical trajectory and render sample); envelope-exhaustion decline behavior; grazing inbox zero-density no-op; new probe (shape of `arrival-earned`) showing a trait-mean tracking a synthetic pressure change
- [ ] Tier-P: instance count and morph/swap amount visibly track a forced pressure change (e.g. Force-panel temperature dial) without an inspector
...
- [ ] **D-007 clip verdict** recorded once shipped — owner judges legible-earned-adaptation vs. arbitrary-effect (C-027-framing.md Owner half)
- [ ] Composition + manifest: `docs/slices/A1-herbivore-composition.md`, `docs/slices/A1.json`
- [ ] **Next-but-one:** A2 — Seed disperser fields (§4.67)
