...
**Next (executable tip):** **Track V §4.62** composite runner-up guild and **Track A §4.67** Seed disperser fields are the two open executable machine slices, running in parallel on disjoint tracks. **§4.66** Herbivore shipped its machine half 2026-08-05 (D-007 clip verdict outstanding — [ask](playtests/A1-herbivore.md) — the gate for any *other* slice registering a new Process; §4.67 extends A1's existing `populations` Process rather than registering a new one and is explicitly not blocked by it, per C-027-framing.md). **§4.60** per-guild silhouettes, **§4.61** per-cell clustering, **§4.64** occupant sway direction correctness, and **§4.65** weather presentation (C-020 G6–G9) are all shipped, as are Track R **§4.48** and **§4.40 L5** (**C-023 Locked**). **§4.63** distance LOD stays queued behind §4.62, gated on profiling. **§4.68** adaptive radiation is a stub gated behind owner Lock on **C-029** and A1+A2 shipping. Track T parked on owner taste. **L8** remains blocked on **C-024** / **C-025**. Keep nutrients / SWE off the tip; animal competition (mesopredator/apex) and ecosystem-engineer roles stay off Track A per C-027-framing.md §4.6/§5.
...
### 4.62 Slice — Plant rendering: composite runner-up guild *(queued — Track V, after §4.61)*
...
**Bans.** No new `WorldState` field. Does not reopen or duplicate **C-023** (guild competition/displacement, Open) — this composites what already coexists in the biomass fields today; it does not simulate competitive displacement.
**New Process?** no. D-007 clip gate does not apply.

- [ ] `OccupantMesh` reads the second-highest `shootVisibility` guild per cell in addition to the arg-max, and draws it at reduced scale when it clears the same visibility floor `shootVisibility` already applies to the winner
- [ ] Tier-P: mixed-stand proxy — a synthetic cell with two guilds above floor shows two non-zero per-guild instance contributions attributable to that cell, not just the winner's
- [ ] Composition + manifest: [plant-render-composite-guild-composition.md](slices/plant-render-composite-guild-composition.md), [plant-render-composite-guild.json](slices/plant-render-composite-guild.json)
- [ ] **Next-but-one:** §4.63 distance LOD (deferred, not queued next — profile first)
...
**A third, parallel queue — Track V, plant rendering presentation.** Owner screenshot (2026-08-02) read the island as "plants all read as thin pyramids" against increasingly legible terrain/water/atmosphere. [Plant rendering review](reviews/2026-08-03-plant-rendering-review.md) (2026-08-03) traced it to one shared `ConeGeometry` standing in for all six vegetation guilds in `OccupantMesh`, one instance per cell, arg-max guild selection discarding the other five fields per cell. The review also weighed the owner's stated fork against the register: animal rendering (**C-027**) is gated behind **F-001** undeferring and **L2–L5** landing, with **L5** itself still blocked on the Open **C-023** — so moving to animals instead was not a same-cost alternative available this week regardless of the plant-rendering question. Owner agreed with the review's recommendation 2026-08-03. Queued as **§4.60–§4.63**: **§4.60** per-guild silhouette geometry (tip) → **§4.61** per-cell clustering → **§4.62** composite runner-up guild → **§4.63** distance LOD (deferred, profile first). None registers a new `Process` (no D-007 obligation) and none needs a candidate — every item reads fields `OccupantMesh` already reads, stays inside T-006's render/sim separation, and stays inside T-007's stock-Three.js stack (no vendored plant assets).
