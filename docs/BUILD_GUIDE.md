...
**Next (executable tip):** **§4.60** vegetation/habitat minor findings — cast-shadow term in `terrainInsolation`, `veg.cover` decay shape (Track R — **§4.48** habitat/dispersal determinism hygiene Done); **L5** blocked on **C-023**, **L8** on **C-024** / **C-025**. **§4.59** / **§4.58** / **§4.57** / **§4.56** / **§4.55** / **§4.54** / **W0** / **L3** / **L7** / **L4** / **§4.45** / **§4.46** / **§4.47** / **§4.48** Done. Track T (terrain tools) has no further machine slice — C-028 structural is fully shipped; remaining C-028 work is owner taste, batched with the owner Lock backlog. Keep nutrients / animals / SWE / wet-sand / freeze off the tip. Cloud succession: [CLOUD_AGENT_PIPELINE.md](CLOUD_AGENT_PIPELINE.md) (Track T parked on owner taste; Track R = §4.60).
...
- [x] `habitatProcess.reads` declares `terrain.elevation` and `soil.material`
- [x] Guild establishment update switches to a Jacobi snapshot — `arrivalComposition.applyGuildEstablishment` reads only pre-call biomass plus already-committed fields per guild spec and writes only that guild's own biomass, removing same-tick order dependence structurally rather than by convention
- [x] `runHerbEstablishmentStep` reads the HSI values `runDispersalStep` already computed and wrote (five new registered fields, `habitat.{strand,binder,marsh,shrub,crust}Suitability`) instead of recomputing them; `vegetationSeasonalProcess.reads`/`dispersalProcess.writes` updated to match, and the now-unused raw-field reads (`shore.exposure`, `soil.salinity`, `soil.moisture`, `soil.material`, `shore.longshore`, `terrain.elevation`) dropped from the former
- [x] Test: six distinct guild specs processed forward and reversed produce byte-identical per-guild results (closes the Symmetry gap directly) — `src/sim/arrival.test.ts`; plus a regression test proving `runHerbEstablishmentStep` no longer recomputes suitability from live fields
- [x] Baselines moved and refreshed (stated in commit body / [composition](slices/habitat-dispersal-hygiene-composition.md)): mechanical `hashN`-only churn across most scenarios (five new registered fields change every full-state hash) — `deep-time`, `regime-divergence`, `branch-compare`, `burn-recover`, `island-drainage`, `tidal-envelope`, `shore-exposure`, `longshore-drift`, `orographic-wind`, `salinity-arrival`, `heat-arrival`, `strand-arrival`, `binder-arrival`, `marsh-arrival`, `spray-arrival`, `inundation-arrival`, `light-arrival`, `island-arrival`, `substrate-contrast`, `substrate-deposit`, `hillslope-deposit`, `cloud-delivery`, `event-band-gate`, `season-regime`, `erosion-intensity`; real (non-hash) magnitude moves from guild growth no longer riding a same-tick recompute of a co-guild's just-updated biomass — `deep-time`, `disturbance-recovery`, `living-hollow`, `shrub-arrival`, `crust-arrival`
- [x] Composition + manifest: [habitat-dispersal-hygiene-composition.md](slices/habitat-dispersal-hygiene-composition.md), [habitat-dispersal-hygiene.json](slices/habitat-dispersal-hygiene.json); **Next-but-one:** §4.60 Vegetation/habitat minor findings (cast shadow, cover decay shape) — the second queue's nine defect-fix slices (§4.44–§4.52) are now fully shipped and §4.60 is the sole remaining unblocked, non-owner-judged item across all six reviews; see its entry for why nothing else qualifies
...
### 4.60 Slice — Vegetation/habitat minor findings *(queued)*

**Why this exists.** [Vegetation/habitat review](reviews/2026-07-31-vegetation-habitat-review.md) §2.5 "Minor" — two of its three findings were never folded into a numbered slice (the third, `factorSandSubstrate`'s missing `clamp01`, shipped under §4.46). With §4.48 landing, the second queue's nine defect-fix slices (§4.44–§4.52, from the four newest reviews) are fully shipped, L5/L8 stay owner-blocked (**C-023**/**C-024**/**C-025**), and Track T has no further machine slice (§4.59) — this is the only remaining item across all six reviews that is both unshipped and not gated on an owner-judged Open candidate. (1) **No cast-shadow term.** `terrainInsolation` (`lightCompetition.ts:22-33`) computes slope/aspect incidence but never tests whether a taller neighboring cell occludes the sun — a cliff-foot cell reads full sun through the cliff, which is exactly the kind of "does the terrain I sculpted matter" signal a sculpting sandbox is supposed to deliver (C-011: real-world intuition is the instrument — the same framing §4.46 already cited for backwards burial/moisture curves). (2) **`veg.cover` decay is linear, not exponential.** `WorldState.ts:1418`'s `cover -= decay * (1 - moisture / thresh)` removes the same absolute amount whether a cell holds 0.99 cover or 0.01 — real dieback is proportional to standing stock. `arrivalComposition.nextHerbBiomass` already fixed this exact shape for guild biomass under **L3** (`biomass -= mortalityRate * excess * dt`, S-007-cited); `veg.cover` itself, the field `runVegetationStep` still updates with the old flat decrement, was out of L3's scope and never revisited.

**Register:** C-011 Open (cast shadow — physical-intuition mismatch, same framing as §4.46; implement nothing that would promote C-011 itself, this is a mechanics correctness fix under it, not a decision about it); S-007 Locked (proportional-to-stock loss, extending L3's already-Locked precedent to `veg.cover`)
**New Process?** no — corrects an existing insolation function and an existing decay term inside `runVegetationStep`. D-007 clip gate does not apply.

- [ ] `terrainInsolation` gains a cast-shadow term — a taller cell along the sun's incoming direction occludes a shorter one behind it; cheap approximation acceptable (a short ray march or horizon-angle check along the dominant light direction), full horizon-shading system not required (GEO-002 earn-its-cost — do not build a general-purpose shadow engine for one cliff-foot case)
- [ ] `veg.cover` decay switches from an absolute decrement to `cover -= decayRate * cover * (1 - moisture / thresh) * dt`-shaped proportional loss, matching `nextHerbBiomass`'s already-Locked mortality shape; verify a near-zero cell doesn't decay past zero and a near-full cell's loss rate visibly differs from a sparse cell's under the same dry spell
- [ ] Test: a cliff-foot cell reads measurably lower insolation than an equivalent unshaded cell at the same slope/aspect; a dense-cover cell and a sparse-cover cell under identical dry conditions lose different *fractions* of a day's cover, not the same absolute amount
- [ ] Baselines: any insolation-derived probe (`succession-diverge`, `light-arrival`, `drydown-feedback`) and any dry-down probe touching `veg.cover` should be checked for real (not just hash) movement and refreshed with a stated reason if so
- [ ] Composition + manifest; **Next-but-one:** none identified — if this closes with nothing else unblocked, say so plainly rather than inventing a tenth queue item; the tip becomes fully owner-gated (Lock sittings for C-014/C-021/C-022/C-023/C-024/C-025/C-026, C-028 taste)
...
| — | Vegetation/habitat minor findings (cast shadow, cover decay shape) | C-011, S-007 | §4.60 **Queued** — sole remaining executable Track R item |
