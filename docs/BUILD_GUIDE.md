...
**Next (executable tip):** **Track A §4.67** Seed disperser fields is the sole open executable machine slice (Track V's **§4.62** composite runner-up guild shipped this session — [composition](slices/plant-render-composite-guild-composition.md); **§4.63** distance LOD stays deferred, first post-composite dual-fill timing **32.23ms** at `config.gridSize`). **§4.66** Herbivore shipped its machine half 2026-08-05 (D-007 clip verdict outstanding — [ask](playtests/A1-herbivore.md) — the gate for any *other* slice registering a new Process; §4.67 extends A1's existing `populations` Process rather than registering a new one and is explicitly not blocked by it, per C-027-framing.md). **§4.60** per-guild silhouettes, **§4.61** per-cell clustering, **§4.64** occupant sway direction correctness, and **§4.65** weather presentation (C-020 G6–G9) are all shipped, as are Track R **§4.48** and **§4.40 L5** (**C-023 Locked**). **§4.68** adaptive radiation is a stub gated behind owner Lock on **C-029** and A1+A2 shipping. Track T parked on owner taste. **L8** remains blocked on **C-024** / **C-025**. Keep nutrients / SWE off the tip; animal competition (mesopredator/apex) and ecosystem-engineer roles stay off Track A per C-027-framing.md §4.6/§5.
