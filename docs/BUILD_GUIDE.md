...
| **L4** | Biotic motion (wind sway) | Shoots lean with wind; calm is still; standing dead barely leans | `occupantSway`, OccupantMesh | **Done** — presentation ([L4-composition](slices/L4-composition.md); D-007 clip Pass); direction/readability corrected in **§4.60** |
...
**Current gate:** Slices **14** / **16** / **15** Tier-O **Pass**; **Slice F** / **17**–**21** Done. Maritime shore Tier-O **Pass** (C-016 / C-017). Salt / overseas Tier-O **Pass**. Stewardship: **C-004** / **C-005 tooling** / **C-013** / **C-002** / **U-006** / **C-020 Locked** (v2.0.13); **C-006 Locked** (CI); **C-014** Open (**hearable** after Wave 0 — owner silence question still outstanding). **Slice G** shipped (machine) — **C-021** / **C-022** wired, both Open pending owner Lock sitting; **C-010** framing Done; **C-028** framing Done (sculpt toolbox). **Slice E** Exner-lite Done. **Slice N8** / **N7** / **N9** / **N10** / **N11** Done. **Slice L1** / **L6** Done; **L2** Done; **L3** Done; **L7** Done; **L4** Done; **§4.44** / **§4.45** / **§4.46** / **§4.47** / **§4.49–§4.52** Done; **Wave 0 (§4.53)** Done; **§4.54 starting surface** Done; **§4.55 brush size** Done; **§4.56 flatten/trowel** Done; **§4.57 mold stamps** Done; **§4.58 Simple/Full chrome** Done; **§4.59 duplicator stamp** Done — C-028 structural kit now fully shipped. **Plant rendering** ([review](reviews/2026-08-03-plant-rendering-review.md)) owner-agreed 2026-08-03: **§4.60** / **§4.61** / **§4.62** Done (per-guild silhouettes, clustering, composite runner-up); **§4.63** deferred pending profiling (first post-composite dual-fill timing **32.23ms** at `config.gridSize`). Gap inventory: [reviews/2026-07-30-sim-gap-review.md](reviews/2026-07-30-sim-gap-review.md). **BUILD_GUIDE “Done” ≠ Lock.** **Queue tip:** Track T (terrain tools) has no further machine slice — remaining C-028 work is owner taste, batched below; Track R's **§4.48** Done; Track V machine queue closed through **§4.62** (§4.63 deferred); **Track A**'s tip is **§4.67** Seed disperser (**§4.66** Herbivore machine half shipped 2026-08-05, D-007 clip verdict outstanding — [ask](playtests/A1-herbivore.md))**; L8 (§4.43) **Done** — **C-024** / **C-025 Locked** (v2.0.21). **Defect closeout (2026-08-06):** white-lag under Heat:warm where water settles was (1) sticky `precipPhase` + snow ground-hold and (2) inland `WaterMesh` puddle-rim foam mixing to near-white — not groundwater ([evidence](evidence/bugs/2026-08-06-white-lag-heat-phase-sticky.md)); Heat dial syncs phase, rain snap-clears the hold, inland foam removed (beach foam stays on ocean). Owner Lock backlog ([owner-lock-batch.md](candidates/owner-lock-batch.md)) — **C-014** now hearable. Nutrients stay off tip; animal life is no longer blanket off-tip — Track A covers non-competitive roles only (mesopredator/apex/engineer roles and **C-029** adaptive radiation stay off tip, see §4.66–§4.68).

**Owner Lock backlog:** ~~A~~ / ~~B~~ / ~~**C-004**~~ / ~~**C-005**~~ / ~~**C-013**~~ / ~~**C-002**~~ / ~~**U-006**~~ / ~~**C-020**~~ / ~~**C-023**~~ / ~~**C-024**~~ / ~~**C-025**~~ **Locked**; **W-001 Superseded**; remaining Open: **C-014** (audio env — beds live, sitting outstanding), **C-021** / **C-022** (Slice G machine half done, taste sitting outstanding), **C-028** (structural kit fully shipped through §4.59 — remaining question is taste: do molds/duplicator feel like shaping sand or placing architecture; do flags/banners earn a chrome slot). Filed Open and owner-judged, none on tip: **C-026** (CVD-safe palette). **C-027** Locked (Track A).

**Next (executable tip):** **Track A §4.67** Seed disperser fields is the open animal-life machine tip (Track V's **§4.62** Done; **§4.63** deferred). **§4.66** Herbivore machine half shipped (D-007 clip outstanding — [ask](playtests/A1-herbivore.md)). **L8** (§4.43) **Done** — deep-time skip + presentation LOD; **C-024** / **C-025 Locked**. Next performance follow-on: SIM §6.5 #2 semi-implicit / fewer flux substeps. **§4.68** adaptive radiation stub gated on **C-029**. Track T parked on owner taste. Keep nutrients / SWE off the tip; animal competition (mesopredator/apex) and ecosystem-engineer roles stay off Track A per C-027-framing.md §4.6/§5.
...
### 4.42 Slice L7 — Activity-gated event band *(Done — agent)*

**Why this exists.** [SIMULATION_MODEL.md](SIMULATION_MODEL.md) §6.2 specifies the event band as **activity-gated** — *"Between storms the ladder starts at `daily`"* — and argues it is determinism-safe because the gate is a pure function of authoritative state. It was **not implemented**: `WorldState.stepEvent` ran the event band unconditionally. Measured payoff ([review](reviews/2026-07-31-time-architecture-review.md) §3): a century falls from **39.9 min to 6.7 min** of CPU at 30 storm-days/year.
...
- [x] Gate predicate: any `water.surfaceDepth > 0` (flux authority — §6.2's `dryEpsilon` is presentation; identity requires following `fluxStep`'s `w <= 0` continue), or armed storm window will discharge precip, or any cell burning — pure function of authoritative state
- [x] **Atmosphere excluded from the gate:** always `runAtmosphereStep()` on skipped spans so dry-day cloud decay and wet-day dawn charge continue; surfaceWater + fire sleep. Stated in [L7-composition.md](slices/L7-composition.md)
- [x] `clock.simMinutes` still advances across gated spans; daily / seasonal / annual / decadal counters unchanged
- [x] **Ship gate: hash-identity.** Probe `event-band-gate`: gated vs ungated over 40 sim-days (light regime, crosses annual at day 36) — `stateHash` identical, precip/cloud identical, skipFrac **0.3** (1152 / 3840). Not a tolerance — identity
- [x] Every existing probe baseline **unmoved** — same claim as hash-identity; CI catches drift
- [x] Composition + manifest ([L7-composition.md](slices/L7-composition.md), [L7.json](slices/L7.json)); **Next-but-one:** L4 biotic motion (§4.39, already specified); L8 remains blocked on C-024 / C-025

---

### 4.43 Slice L8 — Deep-time skip + presentation LOD *(Done — agent, 2026-08-06)*

**Why this exists.** Centuries are not reachable by throughput. Measured cost of one sim-year by integration floor ([review](reviews/2026-07-31-time-architecture-review.md) §3): today **23.95 s** (century = 39.9 min) · activity-gated **3.99 s** (6.7 min) · daily floor **2.03 s** (3.4 min) · seasonal floor **0.40 s** (39.6 s) · decadal floor **0.09 s** (**9.2 s**). Continuous L6 hitch also drove the snow-hold / precip-streak presentation bugs. Deep time needs a **declared** skip floor; hitch needs presentation LOD that sheds theatre without lying about water/terrain/snow-hold.

**Register:** **C-024** Locked; **C-025** Locked; S-009; T-001; T-002; T-003; T-006; U-002; C-008
**New Process?** no — scheduler floor + save schedule + presentation LOD. D-007 clip gate does not apply.

- [x] Discrete skip menu (1 day … 1000 years) beside L6 rates — separate control surface
- [x] Floor-bound advance (`commitSkipDay` / `Seasonal` / `Annual` / `Decadal`); L7 gate still applies inside event floor
- [x] Sparse `skipSchedule` on `SaveDocument` (schema v13); round-trip tested
- [x] HUD names floor during/after skip; presentation LOD tiers P0–P4
- [x] C-024: compressed periods asserted; C-025: seasonal vs annual 1y cover rel err &lt; 0.08
- [x] Composition + manifest: [L8-composition.md](slices/L8-composition.md), [L8.json](slices/L8.json)
- [x] **Next-but-one:** SIM §6.5 #2 semi-implicit / fewer flux substeps (continuous storm hitch) — or Track A §4.67 Seed disperser (independent tip)
...
### 4.46 Slice — HSI curve-shape corrections *(Done)*
...
- [x] Upland inundation gets the supratidal taper marsh already has (`inundationComposition.ts`)
- [x] Intolerant-guild salinity gets a lower plateau on the existing `factorSalinityTolerant` shape (`salinityComposition.ts`)
- [x] Temperature gets an upper limb — unimodal, right-skewed thermal performance curve (`temperatureComposition.ts`)
- [x] Moisture gets a wet-side penalty for herb/shrub, and is inverted (peaks low-to-moderate) for crust specifically (`hsiComposition.ts`, `shrubHsiComposition.ts`, `crustHsiComposition.ts`)
- [x] Binder burial tolerance becomes a hump with optimum at moderate accretion, and the forcing term switches from `|longshore|` (magnitude) to the transport divergence ∂Q/∂x — uniform drift should net zero burial pressure (`binderHsiComposition.ts`)
- [x] Strand/binder exposure gets a destructive upper limb matching marsh's existing hump shape (`strandHsiComposition.ts`, `binderHsiComposition.ts`)
- [x] `factorSandSubstrate` gets `clamp01` — the one unbounded curve in the set of sixteen (`binderHsiComposition.ts`)
- [x] Test: each corrected curve is unimodal or threshold-shaped as specified, bounded [0,1], and the MHW/burial/moisture edge cases named above no longer produce the described backwards result — `hsiCurveShape.test.ts`
- [x] Baselines: guild `*-arrival` and HSI-dependent probes refreshed — reason is the curve fix (half-fill moisture fixtures, mid-exposure crests, TPC upper limb), stated in composition
- [x] Composition + manifest: [hsi-curve-shape-composition.md](slices/hsi-curve-shape-composition.md), [hsi-curve-shape.json](slices/hsi-curve-shape.json); **Next-but-one:** §4.47 Guild cover & light-competition correctness

---

### 4.47 Slice — Guild cover & light-competition correctness *(Done)*
...
- [x] `physicalCoverFrom` and `canopyCoverFraction` switch from additive-clamped to product-complement (`1 − Π(1 − cᵢ)`) via a shared `combineCoverFractions` helper
- [x] Vegetation growth reads absorbed light (light intercepted across the open fraction, `I₀(1 − cover)` under the corrected LAI) instead of transmitted light, removing the double-count with the explicit `(1 − cover)` logistic term
- [x] `LAI = cover · maxLAI` replaced with the Beer–Lambert-consistent inverse `LAI = −ln(1 − cover)/k`; transmitted light is the exact `I₀(1 − cover)` (→ 0 at full cover, no floor), reported LAI clamped to `[0, maxLAI]`
- [x] Test: three guilds at 40% independent cover combine to ≈0.784, not 1.0; full-cover transmitted light is 0, below the old `exp(−k·maxLAI)` floor — `src/sim/guildCoverLight.test.ts`
- [x] Baselines moved and refreshed (stated in commit body / [composition](slices/guild-cover-light-composition.md)): `deep-time` (`meanCover` ~doubles; conservation intact), `living-hollow`, `succession-diverge`, `burn-recover`, `regime-divergence`, `drydown-feedback`, `disturbance-recovery`, `baseflow-persist`, `island-drainage`, `orographic-wind`, `cloud-delivery`, `event-band-gate`, `substrate-contrast`, `substrate-deposit`, and the light/salinity/heat/strand/binder/shrub/spray arrival probes — full-state hashes downstream of the cover/light change; invariant deltas held
- [x] Composition + manifest: [guild-cover-light-composition.md](slices/guild-cover-light-composition.md), [guild-cover-light.json](slices/guild-cover-light.json); **Next-but-one:** §4.48 Habitat/dispersal determinism hygiene
...
- [x] Composition + manifest: [encoding-delta-composition.md](slices/encoding-delta-composition.md), [encoding-delta.json](slices/encoding-delta.json); **Next-but-one:** Wave 0 Locked presentation debt (§4.53)

---

### 4.53 Wave 0 — Locked presentation debt *(Done — machine)*

**Why this exists.** Game-side survey of presentation / session / ecology against THESIS: AUD-001–003 Locked with zero audible output; fire killed `veg.cover` but not guild biomass (OccupantMesh left shoots standing); session started at `1 day/s` with tool look (undo frozen immediately); one forever island. Closes register→build gaps on Locked entries — no new candidate except noting **F-003** does not block same-preserve seed regenerate.

**Register:** AUD-001 / AUD-002 / AUD-003 Locked; **C-014** Open (do not promote); C-008, C-013, T-001, T-006, ES-004, N-004; F-003 Deferred (ok).
**New Process?** no. D-007 clip gate does not apply.

- [x] **0a** Live ambient beds on first gesture; AudioBus mixes drive gains; text-hint stand-ins retired; CI null without AudioContext
- [x] **0b** Seed field + New island; seed in status line; same preserve type
- [x] **0c** `runFireStep` applies `fireVegMortality` to all six `veg.biomass.*`; fireProcess contributes those fields; test in `fire.test.ts`
- [x] **0d** Default `pause` + tool `dig`
- [x] No probe baseline / `GOLDEN_*` moved (stated reason: biomass starts 0 on determinism schedule; burn-recover never establishes guild biomass)
- [x] C-014 dossier updated — beds live; owner question unchanged
- [x] Composition + manifest: [W0-composition.md](slices/W0-composition.md), [W0.json](slices/W0.json); **Next-but-one:** L3 mortality as a rate (§4.38) — already specified
...
- [x] **Next-but-one:** §4.55 sculpt brush size (bucket / shovel) — C-028 framing; parallel tip **§4.46** HSI curve-shape corrections

---

### 4.55 Slice — Sculpt brush size (bucket / shovel) *(Done — agent)*

**Why this exists.** Owner sand-castle toolbox kit ([C-028-framing.md](candidates/C-028-framing.md)): Trusty Bucket vs Heavy Shovel are the first structural tools after §4.54's canvas. One fixed brush radius could not express fine towers vs mass trenches. Ships under Locked **C-006** / **A-005**; wet-sand / freeze / figurines stay banned per the framing.

**Register:** C-006 Locked; C-002 Locked; C-013 Locked; A-005 Locked; C-028 Open (framing only — this cut does not wait on Lock)
**New Process?** no — brush radius parameter on existing berm/dig/deposit. D-007 clip gate does not apply.

- [x] `config.sitingBrushRadii` — `bucket: 4`, `shovel: 8`; `sitingBrushRadius` remains the bucket default
- [x] `raiseBerm` / `digChannel` / `depositSubstrate` take optional `radius`; UI passes the active tier
- [x] Controls: **Brush: bucket** / **Brush: shovel**; cursor footprint tracks size
- [x] Tests: shovel touches more cells than bucket; shovel conserves ΣΔelev = ΣΔdepth (`siting.test.ts`)
- [x] Composition + manifest: [sculpt-brush-size-composition.md](slices/sculpt-brush-size-composition.md), [sculpt-brush-size.json](slices/sculpt-brush-size.json)
- [x] **Next-but-one:** §4.56 flatten / trowel edit (player smooth — distinct from C-022 force dial)

---

### 4.56 Slice — Flatten / trowel edit *(Done — agent)*

**Why this exists.** C-028 kit: Fine Trowels & Palettes — flatten walls, crisp edges, ramparts. Player **edit** that moves elev+depth toward a local target plane inside the brush (C-002), not the C-022 erosion *force* dial and not a nature-applied smooth.

**Register:** C-006 Locked; C-002 Locked; C-013 Locked; A-005 Locked; C-028 Open; C-022 Open (must stay distinct)
**New Process?** no. D-007 clip gate does not apply.

- [x] Flatten tool: toward **mean** elev in brush footprint with berm falloff; depth moves with elev (C-002)
- [x] Undo before time (C-013); no veg write (C-006) — wired in `main.ts` + C-006 edit loop
- [x] Test: after flatten, local elev variance drops; ΣΔelev = ΣΔdepth; hash undo identity (`siting.test.ts`)
- [x] Composition + manifest: [flatten-trowel-composition.md](slices/flatten-trowel-composition.md), [flatten-trowel.json](slices/flatten-trowel.json)
- [x] **Next-but-one:** §4.57 geometric mold stamps (specified below); parallel tip **§4.46** HSI curve-shape corrections
...
**Loops.** Sim: stamp applies a fixed elev(+depth) delta field in a mold footprint; bedrock = elev − depth holds (C-002). Game: pick a mold, click once, get a recognizable rampart / mound / cylinder without painting ecology.

**Register / candidates.** C-006 Locked; C-002 Locked; C-013 Locked; A-005 Locked; C-028 Open (framing — structural); C-009 Locked (deposit stays separate — molds are shape-only unless stamped with deposit).
**Study.** C-028 framing kit map §1; RCT3 brush feel (EXTERNAL_REFERENCES) — steal continuous sculpt feel, not voxel building kits.
...
### 4.59 Slice — Duplicator stamp *(Done — agent)*
...
- [x] `WorldState.copyForm(srcCx, srcCz, radius?)` captures a relative elev(+depth) delta field (and optional material) against the source footprint mean; a pure observer — no sim write
- [x] `WorldState.pasteForm(dstCx, dstCz)` re-applies the captured delta field; depth rides with elev (C-002); undo before time (C-013)
- [x] Copy/paste **never** reads or writes vegetation, water, or suitability (assert the copied buffer excludes those fields, and 20 pastes write no veg — C-006 / N-001)
- [x] Test: paste reproduces the source relief within f32 (ΣΔelev = ΣΔdepth); undo hash identity; copy leaves the world hash unchanged (observer, P-006/T-006)
- [x] Composition + manifest: [duplicator-stamp-composition.md](slices/duplicator-stamp-composition.md), [duplicator-stamp.json](slices/duplicator-stamp.json)
- [x] **Next-but-one:** C-028 structural kit fully shipped — Track T has no further machine slice; owner taste (do molds/duplicator feel like shaping sand?) joins the owner Lock backlog. Sole remaining executable queue: Track R's **§4.48** habitat/dispersal determinism hygiene (**§4.47** shipped, PR #10 merged)

---

### 4.58 Slice — Simple / Full control density *(Done — agent)*

**Why this exists.** Owner: the control bar takes too much real estate. Locked **U-001** already requires a readable first layer with deeper diagnostics on demand; the strip had grown into one wrap-heavy dashboard. This is presentation chrome under U-001 / U-003 — not a new force, not a new Process.

**Loops.** Sim: infrastructure (T-006 — observer only). Game: **Simple** keeps sculpt · Rain/Sea/Wind · time · undo; **Full** reveals secondary force dials, inspect, branch/predict, and session chrome.

**Register / candidates.** U-001 Locked; U-003 Locked; C-004 Locked (forces stay global); T-005 (inspect stays Full); T-006.
**Study.** FORCE_PANEL feel contract; U-001 rejected "one information density for every player and task."
**Bans.** Cell-targeted forces · inspector-as-payoff · inventing Locked density policy beyond U-001 · blocking §4.57 molds.
**New Process?** no. D-007 clip gate does not apply.

- [x] `#chrome-density` Simple | Full toggle; default Simple; preference in `localStorage`
- [x] Controls grouped into loop / session / read rows; `.chrome-full` hidden in Simple
- [x] Simple keeps Rain / Sea / Wind + time + tool/brush + undo; deposit material only when Tool: deposit
- [x] Tier-P: `chromeDensityDelta() ≥ 8` (`chromeDensity.test.ts`, `presentation.proxy.test.ts`)
- [x] Composition + manifest: [chrome-density-composition.md](slices/chrome-density-composition.md), [chrome-density.json](slices/chrome-density.json)
- [x] **Next-but-one:** §4.57 geometric mold stamps (unchanged tip); parallel §4.47 guild cover & light-competition correctness
...
**Register / candidates.** ART-001 Current (stylized, legible silhouettes — not a photorealism push); ART-002 Locked (beauty encourages stewardship); ART-003 Locked (ecological change is visible); T-006 Locked (render stays downstream — no new `WorldState` field, no new read); T-007 Current (stock Three.js `BufferGeometry`, no vendored assets).
**Bans.** No new sim state or `Process`. No vendored/imported plant assets — procedural `THREE.BufferGeometry` only. No per-frame stochastic geometry choice. No change to `occupantEncoding.ts`'s six hues or `occupantSway.ts`'s sway math — shape only.
**New Process?** no — geometry construction inside `OccupantMesh`'s constructor only. D-007 clip gate does not apply.

- [ ] Replace the single shared cone with six distinct small procedural geometries, one per guild: strand/binder/crust as flattened wide, short mats (matching the "mat" language already in `occupantEncoding.ts`'s doc-comments); herb/marsh as thin blade clusters (marsh narrower/taller than herb); shrub as a two-lobe canopy (stacked sphere or double cone) instead of a single spike
- [ ] One `InstancedMesh` per guild (six draw calls total, up from one) — every existing per-instance hook (position, `scaleXZ`/`scaleY`, yaw, L4 wind-lean tilt, `instanceColor`) carries over unchanged onto the new geometries
- [ ] Tier-P: silhouette-distinctness proxy — bounding-box aspect ratio (footprint width : height) for the mat guilds (strand/binder/crust) differs from the upright guilds (herb/marsh/shrub) by a stated floor, so "reads as a mat vs. a spike" is measured, not just eyeballed
- [ ] Composition + manifest: [plant-render-silhouette-composition.md](slices/plant-render-silhouette-composition.md), [plant-render-silhouette.json](slices/plant-render-silhouette.json)
- [ ] **Next-but-one:** §4.61 per-cell clustering
...
- [x] **Next-but-one:** §4.62 composite runner-up guild — **Done** (Track V machine tip closed; §4.63 stays deferred)

---

### 4.62 Slice — Plant rendering: composite runner-up guild *(Done — agent)*

**Why this exists.** [Plant rendering review](reviews/2026-08-03-plant-rendering-review.md) §2b, §3 item 3. `OccupantMesh.updateFrom` picks a single arg-max guild per cell (`OccupantMesh.ts:113–171`); the other five guilds' biomass in that cell contributes nothing to the silhouette. A shrub cell with real herb understory currently shows only the shrub shape. This is the one item in the plant-rendering queue that reads a second field per cell instead of just the max — still only fields `OccupantMesh` already reads, no new `WorldState` field.

**Register / candidates.** T-006 Locked (still presentation-only — no new sim state); ART-003 Locked (mixed stands becoming visible is itself a form of ecological change being visible).
**Bans.** No new `WorldState` field. Does not reopen or duplicate **C-023** (guild competition/displacement — now Locked via L5; still not this slice's job) — this composites what already coexists in the biomass fields today; it does not simulate competitive displacement.
**New Process?** no. D-007 clip gate does not apply.

- [x] `OccupantMesh` reads the second-highest `shootVisibility` guild per cell in addition to the arg-max, and draws it at reduced scale (`RUNNER_UP_SCALE = 0.42`, 1–2 sub-instances) when it clears the same visibility floor `shootVisibility` already applies to the winner
- [x] Tier-P: mixed-stand proxy — a synthetic cell with two guilds above floor shows two non-zero per-guild instance contributions attributable to that cell, not just the winner's (`presentation.proxy.test.ts`)
- [x] Composition + manifest: [plant-render-composite-guild-composition.md](slices/plant-render-composite-guild-composition.md), [plant-render-composite-guild.json](slices/plant-render-composite-guild.json)
- [x] **Next-but-one:** §4.63 distance LOD stays deferred — first post-composite timing at `config.gridSize` dual-fill is **32.23ms** (smoke); revisit LOD only if a real-landscape profile shows a real cost

---

### 4.63 Slice — Plant rendering: distance silhouette LOD *(deferred — do not implement before §4.60–§4.62 ship and are profiled)*

**Why this exists.** [Plant rendering review](reviews/2026-08-03-plant-rendering-review.md) §3 item 4. At the default camera framing (`Scene.ts` starts around `(32, 28, 36)` against the world extent) individual blade/mat detail from §4.60 won't resolve past a few units anyway. Collapsing far cells to a cheaper impostor is only worth doing once §4.60–§4.62 are shipped and measured — building LOD against an unprofiled cost is premature optimization, which is why this item is deferred rather than queued.

**Register / candidates.** T-006 Locked.
**New Process?** no. Not agent-executable yet — no open checklist item until the gate below is met.

- [x] Gate: §4.60–§4.62 shipped; first post-composite timing at `config.gridSize` dual-fill measured at **32.23ms** (smoke) — not yet a license to start LOD; revisit only against a real-landscape profile
...
- [ ] **Next-but-one:** none — do not invent this slice early
...
**Loops.** Sim: unchanged — still zero WorldState writes. Game: every visible shoot now leans the same horizontal direction (perpendicular to wind, composed as `leanQuat.multiplyQuaternions` outermost so the lean axis stays fixed in world space regardless of yaw); the per-cell yaw still spins each low-poly cone's facets for visual variety, it just no longer contaminates the lean direction. `swayTilt` was also rebalanced from a symmetric sine through zero to a held lean (70% of peak amplitude) with a minority flutter (30%) riding on top — a windswept plant leans steady into the wind and trembles a little, it does not swing back through vertical like a pendulum.

**Register / candidates.** D-007 Locked (clip verdict unchanged); T-006; C-011 (motion stays a forcing readout, not decoration).
**Study.** L4 composition ([L4-composition.md](slices/L4-composition.md)) — this slice corrects that mechanism's rendering, not its sim law.
**Bans.** No new WorldState field, no per-cell wind or erosion-direction field (owner chose to keep the existing global wind vector as the lean driver, deferred any local-slope/erosion-angle variant).

**New Process?** no.

- [x] `occupantSway.ts`: `swayTilt` biased toward a held lean (`FLUTTER_FRACTION = 0.3`) instead of a symmetric sine — tilt stays within `[0.4×amplitude, amplitude]`, never crossing back toward vertical
- [x] `OccupantMesh.ts`: yaw and lean composed as quaternions (`leanQuat.multiplyQuaternions(leanQuat, yawQuat)`) so the lean's world-space axis is outermost and independent of the per-cell yaw
- [x] Regression test proving the bug: two cells with different yaw hashes now decompose to parallel world-space lean directions (`cos > 0.999`) under one wind vector — fails against the pre-fix code (measured `cos ≈ 0.53`) (`presentation.proxy.test.ts`)
...
### 4.66 Slice A1 — Herbivore population/trait fields *(machine half shipped 2026-08-05 — D-007 clip verdict outstanding, see [docs/playtests/A1-herbivore.md](playtests/A1-herbivore.md))*

**Why this exists.** Habitat has zero animals today — "life" is six plant-guild biomass fields on the 96×96 grid (`WorldState.ts`), rendered as static colored cones through one `OccupantMesh` `InstancedMesh`. **F-001** (advanced ecosystem-engineer behaviors) moved Deferred → Current by owner decision ([DECISION_REGISTER §15](DECISION_REGISTER.md)); **C-027** ([framing](candidates/C-027-framing.md), now **Locked**) specs the architecture this slice implements: population trait-mean fields per role per cell, updated deterministically toward a pressure-derived target at a demographic-turnover-derived rate — the same shape **L3** ships for biomass mortality — rendered as instanced individuals sampling density (literal count) and trait-mean (bone-scale morph / Foxel rung-swap). Herbivore is C-027 §3.5's fully worked example: every pressure term it needs — terrain slope, `factorTemperature`, `f_inundation` — is already computed by existing terrain/climate/**NS-008** machinery. Per C-027 §4.6.3, this slice also carries the smallest real piece of **ES-007**: a herbivore that never eats is decorative wildlife, which **N-005** forbids, so grazing enters `vegetation`'s biomass fields as a delta-inbox contribution (§11.2), not a bolt-on later.

**Register:** E-004 Locked; E-005 Locked (write-back mechanism already exists, C-027-framing §5 — this slice is a *consumer* of it, not an engineer); ES-006 Locked; ES-007 Locked (this slice's grazing term is its first real touch, C-027 §4.6.3); W-003 Locked; N-003 / N-004 / N-005 Locked; D-001 Locked; T-001 Locked; T-006 Locked; T-007 Current (Foxel is a build-time asset pipeline, not a vendored sim engine — does not trip); C-011 Open; C-003 Open (no stochastic drift); C-019 Locked; **C-027 Locked** (this slice is its first machine implementation)
**New Process?** **yes** — the first `populations` band process in `SimScheduler` ([SIMULATION_MODEL §3.7](SIMULATION_MODEL.md)). **D-007 clip gate applies:** this entry must record a clip verdict before any *other* slice registers a new Process. A2 (§4.67) extends this same Process rather than registering a new one and is not blocked by the clip.

- [x] Register `pop.herbivore.density`, `pop.herbivore.stage.juvenile`/`.stage.adult`, `pop.herbivore.occupancy` per §3.7; band annual, legacy yes
- [x] New field family `pop.herbivore.trait.limbLength` / `.insulation` / `.webbing` (trait-mean) and `pop.herbivore.swap.webbing` (hysteretic latch state, C-027 §3.2/§3.4) — same band/legacy status as density (`insulation` registered `seasonal`, matching its own band)
- [x] Carrying capacity derived each annual step from habitat/resources — **never a stored constant** (**ES-006**); mesopredator/apex predation terms are honestly absent (those roles don't exist yet, C-027 §4.6.1), not a hidden `K`
- [x] Trait update law (C-027 §3.3): `traitMean += traitRate · (pressureOptimum(habitatState) − traitMean) · dt`, `traitRate` derived from this step's own stage turnover — never a hand-tuned constant; mismatch (`|pressureOptimum − traitMean|`, using the *raw*, un-envelope-clamped optimum) enters the mortality/capacity term so adaptation costs something (E-006/E-009); `traitMean` itself clamps to a per-species envelope, population declines rather than exceeding it — verified 6.78×10⁻⁹ ind/km² vs. 1.538 ind/km² after 60 annual bands, steep vs. flat terrain, same founder population ([composition](slices/A1-herbivore-composition.md))
- [x] Plasticity/adaptation split (C-027 §3.3): `insulation` is reversible on the `seasonal` band; `webbing` is slow/hysteretic on the `annual` band — two derived rates on two bands, not one law
- [x] Three pressure mappings, each with a real-world referent (**C-011**): `limbLength` ← terrain slope factor (continuous, bone-scale); `insulation` ← `climate.airTemperature` (`temperatureComposition.ts`'s own driving field, continuous, bone-scale, seasonal); `webbing` ← tidal hydroperiod (**C-016**, discrete rung-swap at a two-value hysteretic threshold, annual)
- [x] Grazing write-back (C-027 §4.6.3): herbivore density contributes a mortality-rate term to `veg.biomass.herb` via `Process.contributes` (`vegetation` stays the sole owner); zero herbivore density is a no-op against today's baselines by construction (same zero-is-a-no-op discipline **L2** proved) — measured 2.000 → 1.786 kg DM/m² at density 8.925 ind/km²
- [x] Density target reads existing vegetation-biomass fields within a home-range neighborhood — no new resource field invented (C-027 §3.5)
- [x] Render: one new `InstancedMesh` (`HerbivoreMesh`, sibling to `OccupantMesh`'s pattern); instance count = `density × cellAreaKm2` capped at a render budget — literal readout, never tuned (C-027 §2); per-instance deterministic seed from `(cellIndex, instanceIndex)` only, no persisted identity across frames (**T-001**, **T-006**)
- [x] Base mesh: **placeholder** (owner-approved scope for this slice) — no Foxel dependency; only `limbLength` has an accepted animal-design card ([docs/animal-design/PROTOCOL.md](animal-design/PROTOCOL.md)), so only it drives a visual difference (whole-instance Y-scale, an honest bone-scale approximation, not literal per-bone scaling — a static merged geometry has no skeleton). `insulation`/`webbing` render halves wait on their own cards per the Track A prompt rule; sim-field halves do not
- [x] Tier-M: bounds on density and trait-mean fields (favorable **and** unmeetable-pressure conditions); determinism (identical seed/state → identical trajectory, `stateHash` match); envelope-exhaustion decline behavior; grazing write-back zero-density no-op + nonzero-density effect; new `herbivore-drift` probe (shape of `arrival-earned`) showing insulation tracking a synthetic pressure change (0.133 → 0.933 across a Heat-dial swing, replay-matched) — [composition](slices/A1-herbivore-composition.md)
- [x] Tier-P: `herbivore-drift` probe quantifies the pressure-tracking signal before any owner question is asked (VERIFICATION_POLICY §4) — instance-count/render-side visibility is necessarily sparse at Habitat's ~0.92 km² preserve scale and realistic densities (expected ≈0.0025 instances/cell at full capacity), a property of the literal-readout decision, not a defect
- [x] Inspector overlay: `herbivoreDensity` layer registered and inspectable (**T-005**), verified in the running app (no console errors, layer renders)
- [x] Notebook seed: "The herd here has gone shaggy since the cold set in." (recorded in composition + manifest; U-006 UI itself unchanged)
- [ ] **D-007 clip verdict** — owner judges legible-earned-adaptation vs. arbitrary-effect (C-027-framing.md Owner half); ask filed at [docs/playtests/A1-herbivore.md](playtests/A1-herbivore.md), gate for every later Track A slice registering a new Process
- [x] Composition + manifest: `docs/slices/A1-herbivore-composition.md`, `docs/slices/A1.json`
- [x] **Next-but-one:** A2 — Seed disperser fields (§4.67, specified below to §4.3 depth)

---

### 4.67 Slice A2 — Seed disperser fields *(specified to §4.3 depth — queued behind A1)*

**Why this exists.** Second role in C-027 §3.1's list; the framing names it the biological analog of what **L2** already ships for plants. Natural second cut after A1 proves the `populations` Process end-to-end, because it reuses A1's field/render machinery wholesale and adds exactly one new coupling: a local term in the L2 seed-rain kernel (`dispersalProcess`, [L2-composition.md](slices/L2-composition.md)) so disperser density modulates local seed pressure alongside the existing biomass-driven term. This is also the slice that introduces the first real dispersal-reach/home-range concept for fauna — the piece **C-029** (adaptive radiation, framing) needs before regional isolation is even definable. Pollinators (C-027 §3.1) are architecturally identical to this slice minus the dispersal write-back and are the fallback swap if the coupling proves harder than expected.

**Register:** C-007 Locked; C-019 Locked; E-004 Locked; W-003 Locked; ES-006 Locked; T-001 Locked; C-011 Open; C-003 Open; **C-027 Locked**
...
- [ ] Explicit dispersal-reach parameter per role (home range / kernel radius) — first fauna use of this concept; **C-029** cites this field directly as its own isolation definition's dependency
...
- [ ] Render: same `OccupantMesh`-pattern instancing as A1, new archetype; base mesh sourced from the animal-design lane the same way as A1
...
- [ ] Bans: no stochastic dispersal (**C-003** Open); no predator/competition term (**ES-007**/§4.6 — out of scope); no species catalog, roles only (**E-004**)
- [ ] Composition + manifest: `docs/slices/A2-seed-disperser-composition.md`, `docs/slices/A2.json`
- [ ] **Next-but-one:** §4.68 adaptive radiation stub becomes reviewable once this ships; owner choice among remaining non-predator roles (pollinators next-safest) otherwise. Mesopredator / apex-predator / ecosystem-engineer roles stay blocked behind C-027 §4.6 / §5's respective gaps

---

### 4.68 Slice — Adaptive radiation: regional ecomorph divergence *(stub — gated behind C-029 Lock, not started)*

**Why this exists.** **C-029** ([framing](candidates/C-029-framing.md), Open) proposes that a role's trait-mean field, once genuinely isolated across separated habitat patches under released competitive/predation pressure, may diverge into visibly distinct named regional morphs — the Anolis-ecomorph / Darwin's-finch pattern, scoped to stay inside **W-003**'s fixed species pool (expression diverges, identity does not). Needs **A2**'s dispersal-reach concept to define isolation and at least one owner walkthrough of a real diverging population before this stub can be specified to full checklist depth.

**Register:** C-029 Open (owner Lock required before this slice starts); C-027 Locked; C-019 Locked; W-003 / N-003 Locked
**New Process?** Not yet determined — likely no (extends the existing `populations` Process with a derived clustering field, same posture as A2 extending A1).

- [ ] Gate: **A1** + **A2** shipped, **C-029** owner-reviewed and Locked
- [ ] Not specified further — do not implement ahead of the C-029 Lock
...
| L8 | Deep-time skip + presentation LOD | C-024, C-025, S-009, T-001, T-003 | §4.43 **Done** — skip menu + schedule in save; LOD P0–P4 |
...
| — | Occupant sway direction correctness | D-007, T-006, C-011 | §4.60 **Done** — agent ([composition](slices/occupant-sway-direction-composition.md)) |
...
L1 throughput defect [Done]  →  L6 real-world time units [Done]  →  L2 local seed rain [Done]
   ...
   (L5 Done / **C-023 Locked**; **L8 Done** — C-024 / C-025 Locked)
```

L1 and L6 led because they are small, move no baselines, and are how L2/L3 get observed at all — both are about ecological timescales; both shipped in one commit with every baseline and `GOLDEN_*` hash unmoved (§4.36 / §4.41). **L3** shipped next: first-order dieback, `dieback-lag` probe, guild-ordered half-lives. **L1, L2, L3, L4, L6, L7, L8 register no new `Process`** (D-007 clip gate does not apply). **L5** Locked via **C-023**; **L8** Locked via **C-024** + **C-025**. Owner Lock backlog runs in parallel: residual **C-014**, **C-021**/**C-022** taste sitting, **C-010** implement later ([owner-lock-batch.md](candidates/owner-lock-batch.md)). Keep nutrients / SWE off the tip.

**Two standing risks recorded.** (1) L2 and L3 introduce per-band rate constants under the Locked compressed calendar (**C-024**) — retune only if periods change again. (2) ~~Silent partial deep-time shortcuts~~ — **L8** ships declared skip floors + schedule instead.

**A second, parallel review queue** — four scoped domain reviews using the same expert-review pattern as the renderer ([3c4b9f0](https://github.com/mrfootandmrbear/Habitat/commit/3c4b9f0)): [fire/fuel](reviews/2026-07-31-fire-fuel-review.md), [vegetation/habitat](reviews/2026-07-31-vegetation-habitat-review.md) (extends the living-world review's L2/L3/L5 territory), [hydrology/geomorphology](reviews/2026-07-31-hydrology-geomorphology-review.md), and [UI encoding](reviews/2026-07-31-ui-encoding-review.md) — filed as **§4.44–§4.52**. None registers a new `Process` and none needs a candidate except **C-026** (deliberate CVD-safe palette, Open, owner-judged, which did not block §4.52's bug fixes). **§4.44–§4.47 and §4.49–§4.52 have shipped**; **§4.48** (habitat/dispersal determinism hygiene) remains queued on the parallel review track; terrain-tools wave is fully shipped through **§4.59** duplicator (**§4.54–§4.59** all Done) — Track T has no further machine slice, only owner taste. Three of the shipped five are worth recording outside severity ranking: **§4.44 fire spread as a rate** turned out to be one root cause wearing three faces — nothing tracked which cells were alight between calls, and that single absence produced the missing rate, the intensity that outlived the fire, and the scan-order-dependent burn shape, so tracking the front explicitly closed all three at once; **§4.49 drainage flat-routing correctness** was upstream of more than its own review scope suggested — it corrupted `aNorm`, which both hillslope erosion and the groundwater channel boost depend on, and its fix moved the baseline family §4.51 then moved again; and **§4.52 encoding delta correctness** repaired the Tier-P proxy mechanism ([VERIFICATION_POLICY.md](VERIFICATION_POLICY.md)) that discharges Definition-of-done row 2 for every slice, past and future.

**A third, parallel queue — Track V, plant rendering presentation.** Owner screenshot (2026-08-02) read the island as "plants all read as thin pyramids" against increasingly legible terrain/water/atmosphere. [Plant rendering review](reviews/2026-08-03-plant-rendering-review.md) (2026-08-03) traced it to one shared `ConeGeometry` standing in for all six vegetation guilds in `OccupantMesh`, one instance per cell, arg-max guild selection discarding the other five fields per cell. Owner agreed with the review's recommendation 2026-08-03. **§4.60** / **§4.61** / **§4.62** shipped (per-guild silhouettes, clustering, composite runner-up); **§4.63** distance LOD stays deferred — first post-composite dual-fill timing **32.23ms** at `config.gridSize`, revisit only if a real-landscape profile shows a real cost. None registered a new `Process` (no D-007 obligation) and none needed a candidate — every item reads fields `OccupantMesh` already reads, stays inside T-006's render/sim separation, and stays inside T-007's stock-Three.js stack (no vendored plant assets). Track V has no further machine slice until §4.63's gate is met.
