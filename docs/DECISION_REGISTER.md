...
8m. **C-027** — animal trait expression as population fields (procedural morph + threshold swap); filed Open from an owner design chat, framed in [C-027-framing.md](candidates/C-027-framing.md). Gated behind **F-001** undeferring and **L2**–**L5** landing for the plant substrate — not tip.
...
**Constraints.** No individual entity/identity store (**T-001**, **T-006**). No stochastic trait drift while **C-003** is Open — target must be a deterministic function of pressure fields. No fixed carrying-capacity constant (**ES-006**). No player-authored creature bodies — species identity stays resolved by **W-003**'s fixed pool; only trait expression is free-running (**N-003**, **N-005**, **D-001**). Every trait/pressure mapping needs a real-world referent (**C-011**, **N-004**). Cross-role predation may not skip a trophic level (herbivore ← mesopredator ← apex only, never apex ← herbivore directly) or it stops being the food web **ES-007** requires.
...
**Leading direction.** File as Open; implement nothing under it. Framing: [C-027-framing.md](candidates/C-027-framing.md) — trait-mean fields per role per cell, first-order-toward-pressure-target update (same shape as L3's mortality-as-rate direction), rendered via an extended `OccupantMesh`-style `InstancedMesh` sampling density (literal instance count) and trait-mean (morph/swap) deterministically per instance. Worked example: herbivore, with limb length ← terrain ruggedness, insulation ← temperature, webbing ← inundation fraction (threshold). §4 adds the trophic backbone: producers → herbivores/pollinators/dispersers → mesopredators → apex predators, decomposers closing the loop, engineers cross-cutting. **L2**–**L5** have landed; **F-001** undeferring is the sole remaining gate — do not implement.
