...
**Constraints.** No individual entity/identity store (**T-001**, **T-006**). No stochastic trait drift while **C-003** is Open — target must be a deterministic function of pressure fields. No fixed carrying-capacity constant (**ES-006**), and no hand-tuned trait rate for the same reason — response speed derives from the role's own demographic turnover, not an authored constant. Adaptation must cost something: trait mismatch enters the role's mortality/capacity term, so a population cannot morph out of a pressure with its density untouched (**E-006**, **E-009**). Trait drift is bounded by a per-species envelope from **W-003** preserve data — a population that exhausts its envelope declines rather than continuing to morph. No player-authored creature bodies — species identity stays resolved by **W-003**'s fixed pool; only trait expression is free-running (**N-003**, **N-005**, **D-001**). Every trait/pressure mapping needs a real-world referent (**C-011**, **N-004**). Must not smuggle in animal population competition/predation dynamics, which is **ES-007**/**C-023**/L5's separate, later scope.

**Leading direction.** File as Open; implement nothing under it. Framing: [C-027-framing.md](candidates/C-027-framing.md) — trait-mean fields per role per cell, first-order-toward-pressure-target update (same shape as L3's mortality-as-rate direction), rendered via an extended `OccupantMesh`-style `InstancedMesh` sampling density (literal instance count) and trait-mean (morph/swap) deterministically per instance. Worked example: herbivore, with limb length ← terrain ruggedness, insulation ← temperature, webbing ← inundation fraction (threshold). The framing separates *plasticity* from *adaptation* — reversible, seasonal-band traits (insulation) vs. slow, hysteretic, annual-band ones (webbing) — since one law at one rate necessarily makes one of them read wrong; threshold swaps latch on two values rather than one, so a morph cannot flicker. §4 / §5 record what the document deliberately does not resolve: prey-dependent roles are unspecifiable until **ES-007** opens, trophic coupling would resolve as a declared one-band lag, herbivory is itself a `veg.biomass` write-back (so the chosen worked role is the one that cannot stay decoupled — **N-005**), apex readiness has no habitat-history field to read, and the **E-005** engineer write-back path already exists and is test-enforced (`Process.contributes` + §11.2 inbox), leaving only band-crossing latency as its real constraint. Gated behind **F-001** undeferring and **L5** landing for the plant substrate (**L2** / **L3** / **L4** shipped) — do not implement.
