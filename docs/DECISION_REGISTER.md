...
**v2.0.19 — F-001 undeferred; C-027 Locked; Track A (animal life) opened.**

- Owner lifted the animal-life block. **F-001** (§15) moved Deferred → Current. Ecosystem-engineer write-back — F-001's literal historical scope — stays the one genuinely undesigned piece (which roles are engineers, what each contributes); the write-back *mechanism* itself (`Process.contributes` + the SIMULATION_MODEL §11.2 delta inbox) already exists and is test-enforced (`src/sim/ownership.test.ts`), so F-001's "extensible write-back path" requirement was already satisfied before this entry, only unused.
- **C-027** promoted Open → **Locked** (§16.5). Its plant-substrate gate (**L2**–**L5**) was already fully discharged — **L5**/**C-023** Locked and shipped 2026-08-03 — leaving **F-001** as the only remaining gate, now also cleared. Body revised: trait-change rate derives from the role's own demographic turnover (`pop.<role>.stage[k]`), not a hand-tuned constant, for the same **ES-006** reason a fixed carrying-capacity constant is banned; mismatch between trait-mean and pressure-optimum enters the role's own mortality/capacity term, so adaptation costs something (**E-006**, **E-009**) instead of being free; drift is bounded by a per-species trait envelope from **W-003** preserve data, and a population that exhausts its envelope declines rather than continuing to morph; threshold swaps latch on two values (attach/detach) with per-cell state in a new `pop.<role>.swap.<name>` field, so a morph cannot flicker on a pressure sitting near the line; plasticity (reversible, seasonal-band — e.g. insulation) and adaptation (slow, hysteretic, annual-band — e.g. webbing) are named as different processes that must not share one law. Render-side settled: continuous traits ride a Foxel-exported glTF skeleton's runtime bone-scale; discrete traits pick a rung from a Foxel-baked `.glb` variant ladder, selected — never blended — by the trait-mean field, since Foxel has no blend shapes; this replaces the original's more hypothetical "morph-target influence" phrasing with the mechanism actually available.
- Opened **Track A (animal life)** — [BUILD_GUIDE §4.66–§4.67](BUILD_GUIDE.md), [CLOUD_AGENT_PIPELINE.md](CLOUD_AGENT_PIPELINE.md) §3. **A1 Herbivore** (§4.66, tip): first `populations` Process, D-007 clip gate applies. **A2 Seed disperser** (§4.67, next-but-one): extends the same Process. Mesopredator/apex-predator roles (food-web coupling, **ES-007**) and ecosystem-engineer roles (write-back role-selection) stay off Track A — undeferring **F-001** opens trait expression for non-competing roles, not the whole entry's literal scope.
- New low-cost parallel design lane opened alongside: [docs/animal-design/PROTOCOL.md](../animal-design/PROTOCOL.md), mirroring `nature-study`'s card mechanism, for Foxel silhouette/variant-ladder proposals — design artifacts only, not shipped assets; a separate, higher-cost step turns an accepted card into a real `.fxl`/`.glb`.

**v2.0.18 — Plant rendering review; queued as §4.60–§4.63 (Track V), no candidate.**

- Owner screenshot (2026-08-02) read the island as "plants all read as thin pyramids" against increasingly legible terrain/water/atmosphere, and named an explicit fork: keep investing in plant rendering, or move on to animals. [Plant rendering review](reviews/2026-08-03-plant-rendering-review.md) (2026-08-03) traced the read to `OccupantMesh` drawing all six vegetation guilds (herb/strand/binder/marsh/shrub/crust) from one shared `ConeGeometry`, one instance per cell, arg-max guild selection discarding the other five biomass fields per cell.
- The review weighed the fork against the register rather than leaving it to taste: animal rendering (**C-027**) is gated behind **F-001** undeferring and **L2–L5** landing for the plant substrate, with **L5** itself still blocked on the Open **C-023** — so "move to animals instead" was not a same-cost alternative available this week regardless of the plant-rendering question. Owner agreed 2026-08-03 with the recommendation to ship plant rendering now.
- **No new candidate.** Same reasoning as the four 2026-07-31 reviews (v2.0.15): every proposed item is presentation-only under Locked **T-006** (no new sim state, no new `Process`, no D-007 obligation) and stays inside Current **T-007**'s stock-Three.js stack (no vendored assets) — a defect against **ART-001**/**ART-002**/**ART-003** falling short in practice, not a new design question. Queued directly as [BUILD_GUIDE.md](BUILD_GUIDE.md) §4.60–§4.63 on a new Track V: **§4.60** per-guild silhouette geometry (tip) → **§4.61** per-cell clustering → **§4.62** composite runner-up guild → **§4.63** distance LOD (deferred until §4.60–§4.62 ship and are profiled).

**v2.0.17 — C-028 filed (sculpt toolbox vocabulary).**

- Filed **C-028** (sculpt toolbox vocabulary — sand-castle craft → Habitat causes) Open after an owner kit-out of buckets, molds, shovel, watering can, trowels, carving tools, beachcomber stash, flags, props, duplicator, sponge, and freeze spray. Framing: [C-028-framing.md](candidates/C-028-framing.md). Structural form tools (brush size, later flatten/molds/duplicator) ship under Locked **C-006** / **A-005**; wet-sand sculpt physics, freeze-against-tide, and placeable life are hard bans (THESIS §2.2, **C-004**, **N-001** / **C-007**); watering can reframes to Force rain; undo (**C-013**) covers misclick take-back. First cut: BUILD_GUIDE §4.55 brush size.

**v2.0.16 — C-027 filed (animal trait-expression framing).**

- Filed **C-027** (animal trait expression as population fields — procedural morph + threshold swap) Open after an owner design chat on how animal archetypes and visual "evolution" should work. Reconciled against **E-004**/**W-003** (role-first, fixed species pool), **T-001**/**T-006** (determinism, no render-authoritative state), and **ES-006** (no fixed carrying-capacity constant): the chat's individual-creature-with-lifetime-mutation assumption is replaced by population trait-mean fields, extending [SIMULATION_MODEL.md](SIMULATION_MODEL.md) §3.7's already-spec'd `pop.<role>.density`/`stage[k]`. Framing: [C-027-framing.md](candidates/C-027-framing.md). Gated behind **F-001** (Deferred) undeferring and **L2**–**L5** landing for the plant substrate first — explicitly off the build tip, same as every other animal item in the queue.

**v2.0.15 — Fire/fuel, vegetation/habitat, hydrology/geomorphology, and UI encoding reviews; C-026 filed.**

- Four scoped domain reviews, same expert-review pattern as the renderer ([3c4b9f0](https://github.com/mrfootandmrbear/Habitat/commit/3c4b9f0)): [fire/fuel](reviews/2026-07-31-fire-fuel-review.md), [vegetation/habitat](reviews/2026-07-31-vegetation-habitat-review.md), [hydrology/geomorphology](reviews/2026-07-31-hydrology-geomorphology-review.md), [UI encoding](reviews/2026-07-31-ui-encoding-review.md).
- Filed **C-026** (CVD-safe cross-domain palette) Open after the UI encoding review measured a sand-binder/intertidal color collision and a red–green CVD confusion pattern across most occupant guild colors. The mechanical cross-file contrast check that would have caught the collision ships without this candidate; choosing replacement hues is the owner-judged part.
- All other findings across the four reviews are **defects against already-Done slices or already-Locked entries, not new candidates**, and are queued as [BUILD_GUIDE.md](BUILD_GUIDE.md) §4.44–§4.52: fire has no rate of spread and leaks stale `fire.intensity` (shortfall against Slice 10's own determinism/conservation claims); fuel accumulation and scar decay are `dt`-fragile (§2.1 Refinement class); six HSI suitability curves are monotone where the physically correct hump/threshold shape already exists elsewhere in the same file set; guild cover stacks additively where it should be product-complement, and light competition uses transmitted rather than absorbed light; a flat-routing bug in Priority-Flood creates 2-cycles that sever the drainage network and corrupt everything downstream that reads `aNorm`; the explicit surface-flux scheme has no CFL-style stability bound; coastal flow uses bed elevation instead of `seaLevel`, and coastal erosion ignores the substrate table hillslope erosion already respects; and the encoded-delta functions that discharge Definition-of-done row 2 ("Observable") for every slice saturate across roughly half their domain, so passing Tier-P proxies are weaker evidence than the project's verification policy assumes.
...
**Status:** Current *(undeferred 2026-08-05 — owner decision)*

*Why deferred (historical):* implementing many species that alter terrain or hydrology may exceed first playable scope.
...
*Why undeferred:* the owner lifted the animal-life block outright. Scope stays split, not wholesale: **C-027** (now Locked) opens non-competitive, non-write-back roles — pollinators, seed dispersers, herbivores — as population trait-mean fields, starting with Herbivore ([BUILD_GUIDE §4.66](BUILD_GUIDE.md)). The literal content of this entry — ecosystem-engineer write-back — stays the hardest, least-designed piece: the write-back *mechanism* (**E-005**'s owned-property/delta-inbox path, `Process.contributes` + SIMULATION_MODEL §11.2) already exists and is test-enforced, but *which* animal roles are engineers and what each contributes is not specified anywhere yet. Mesopredator/apex-predator roles (food-web coupling, **ES-007**) are architecturally reachable now that **L5**/**C-023** is Locked, but still need their own framing pass before a slice may build them (C-027-framing.md §4.1) — undeferring this entry does not, by itself, unlock them.
...
8n. **C-028** — sculpt toolbox vocabulary (sand-castle craft → Habitat causes); filed Open from an owner kit-out, framed in [C-028-framing.md](candidates/C-028-framing.md). Structural form tools under Locked **C-006**; wet-sand physics / freeze-against-tide / placeable life banned; §4.55 brush size is the first cut.
...
**Owner Lock (2026-08-03, delegated in session — "pick the best option").** The leading direction below, adopted as written: shrub is the one structurally taller (woody) guild among the six (DESIGN_WIKI §4), so it is the overstory and the other five are its understory — a real stratification, not an invented ranking, and the one a person's own intuition already contains ("bushes shade the grass underneath them," C-011). Scoped to shrub → herb only for this Lock, not a full six-guild dominance web: herb is the one guild whose Liebig HSI already carries an open-sky light arm (`factorLight`) for the mechanism to attenuate; strand/binder/marsh/crust have no light arm today and are unaffected. Machine half (CI-judged: displacement direction, determinism, single-authority, ES-006 guard) shipped under **L5** — `succession-displace` probe: on identical terrain/seed/warm forcing, herb rises to a peak (≈1.69) then *declines* to ≈0.98 as shrub's own cover rises to ≈1.52, while a no-competition twin (shrub seed suppressed, otherwise identical) shows herb rising monotonically to ≈2.08 and never declining — the suppression is caused by shrub's shade and nothing else. Owner half of the Judge (does displacement read as succession, not a hidden ranking) is satisfied by the mechanism itself: it is the light budget §4.47 already made correct, not a new comparison table, and the curve it produces — pioneer rises, canopy closes over it, pioneer recedes to a shaded remnant — is exactly the successional story a person already expects from watching a field grow up into scrub.

**Leading direction (implemented).** Taller guild attenuates understory light for the shorter guild via the existing Beer–Lambert `evaluateLight`; the shorter guild reads that attenuated value instead of open-sky insolation in its HSI — displacement is a consequence of the light budget already simulated, not a new rule. Landed after **L2** (local seed rain) and **L3** (mortality as a rate), both shipped: without a mortality rate there would be no mechanism by which a suppressed guild could actually recede. Composition: [succession-displace-composition.md](slices/succession-displace-composition.md).
...
**Question.** Should the occupant/guild and terrain palettes be deliberately redesigned so no two categorical states that can co-occur on screen are confusable under common color-vision deficiency (deuteranopia/protanopia), and — separately from taste — should a cross-file contrast check become a standing part of the encoding proxy suite?

**Why now.** Measured in [reviews/2026-07-31-ui-encoding-review.md](reviews/2026-07-31-ui-encoding-review.md) §3: sand-binder mat (`occupantEncoding.ts`) and intertidal foreshore (`terrainEncoding.ts`) sit ~0.07 unit-RGB apart despite being two unrelated quantities that co-occur on the shore, because no existing test compares palettes across files. More broadly, most occupant guild colors vary primarily along the red–green axis — the textbook brown/green CVD confusion pair — while `lightEncoding.ts`'s dark-blue-to-warm-pale ramp is already CVD-robust and is the better template. The mechanical fix (a cross-file contrast check in the encoding proxy suite) ships without this candidate as part of [BUILD_GUIDE §4.52](BUILD_GUIDE.md); choosing the actual replacement hues for six guilds plus terrain substrates is art direction, not a bug fix.

**Constraints.** **U-003** — the world is the primary visualization, so any repalette must still read correctly as a place, not just as a legend. **D-007** — a repalette large enough to touch the default view needs a clip-test pass, same as any other legibility-facing change. Must not become a full art-direction pass smuggled in under an accessibility label — scope is the specific collisions and axis-confusion the review measured, not a general palette rewrite.

**Leading direction.** File as Open; implement nothing under it. If adopted, the binder/intertidal collision (the concrete, measured instance) is the smallest defensible first cut; a full guild-palette rebalance onto a CVD-safe axis (following `lightEncoding.ts`'s existing blue–yellow template) is the fuller version and is more clearly an owner taste call.

### C-027 — Animal trait expression as population fields (procedural morph + threshold swap)
**Status:** Locked (2026-08-05 — see Lock note below)

**Question.** When animal roles are eventually simulated (**F-001** currently Deferred), should each population's morphology be represented as trait-mean fields per cell — updated deterministically from the same local pressure terms vegetation HSI already computes, and rendered as instanced individuals sampling those fields — rather than as discrete creature objects with individually tracked identity or lifetime mutation?

**Why now.** An owner design conversation (2026-07-31) worked through how animal archetypes and visual "evolution" should function and converged on a hybrid model: a fixed role/species frame (already Locked — **E-004**, **W-003**) expressing fluid, environmentally-driven trait values (procedural morph for continuous change, discrete socket-mesh swap for threshold change). That maps cleanly onto [SIMULATION_MODEL.md](SIMULATION_MODEL.md) §3.7's already-spec'd `pop.<role>.density`/`stage[k]` fields and the render-instancing pattern `OccupantMesh` already ships for vegetation — but the chat's own assumption (individual creatures with persistent identity, mutating over a lifetime) does not, and would require an entity-store architecture nothing else in the sim uses. Recording the reconciliation now, while animals are still Deferred (**F-001**) and explicitly off the build tip (AGENTS.md), means the architecture is ready rather than re-derived whenever **F-001** is undeferred.

**Constraints.** No individual entity/identity store (**T-001**, **T-006**). No stochastic trait drift while **C-003** is Open — target must be a deterministic function of pressure fields. No fixed carrying-capacity constant (**ES-006**), and no hand-tuned `traitRate` for the same reason — the speed of adaptation is derived from the role's own demographic turnover (`pop.<role>.stage[k]`), never authored. No player-authored creature bodies — species identity stays resolved by **W-003**'s fixed pool; only trait expression is free-running (**N-003**, **N-005**, **D-001**). Every trait/pressure mapping needs a real-world referent (**C-011**, **N-004**). Cross-role predation may not skip a trophic level (herbivore ← mesopredator ← apex only, never apex ← herbivore directly) or it stops being the food web **ES-007** requires. **No adaptation without a survival cost** — trait mismatch enters the role's own mortality/capacity term, so a population cannot morph its way out of a pressure while its density stays untouched (**E-006**, **E-009**). **No unbounded trait drift** — every trait carries a per-species envelope from **W-003** preserve data; a population that exhausts its envelope declines rather than continuing to morph. **No bare threshold** for a discrete swap — a two-value latch (attach/detach) with per-cell state in `pop.<role>.swap.<name>`, so a morph cannot flicker on a pressure field sitting near the line.

**Update (2026-08-03, owner in session).** Retain the food web as the organizing structure, keep every role terrain-adaptive, keep the render payoff — narrow everything else. **L5** shipped and **C-023 Locked** the same session, so the earlier constraint deferring "animal population competition/predation dynamics" to L5/C-023's scope no longer applies as written: L5 governs *plant* guild competition; this entry's own **ES-007** food-web backbone (bottom-up capacity, top-down predation-as-mortality-rate reusing L3's shape) is now framed in [C-027-framing.md](candidates/C-027-framing.md) §4, not deferred elsewhere.

**Lock (2026-08-05, owner).** **F-001** undeferred the same session (§15) — the sole remaining gate on this entry is now clear, alongside the already-landed plant substrate (**L2**–**L5**). Refined per the latest [C-027-framing.md](candidates/C-027-framing.md): trait-change rate derives from demographic turnover, not a tuned constant; mismatch between trait-mean and pressure-optimum feeds the role's own mortality/capacity term, so adaptation is never free; drift is bounded by a per-species trait envelope, and a population that exhausts it declines instead of continuing to morph; plasticity (reversible, `seasonal`-band — e.g. insulation) and adaptation (slow, hysteretic, `annual`-band — e.g. webbing) are named as different processes that must not share one law; threshold swaps latch on two values with per-cell state. Render mechanism settled (superseding the original's more hypothetical "morph-target/bone-scale" phrasing): Foxel produces the asset, not the simulation — continuous traits ride a Foxel-exported glTF skeleton's runtime bone-scale (an ordinary Three.js capability, independent of Foxel's own lack of blend shapes); discrete traits pick a rung from a Foxel-baked `.glb` variant ladder, selected — never blended — by the trait-mean field. §4/§5 record two gaps this entry still leaves open rather than resolving: only abiotically-keyed roles (herbivore, pollinator, seed disperser) are specifiable before a second, prey-linked role exists; and the **E-005** write-back *mechanism* (`Process.contributes` + the §11.2 delta inbox) already exists and is test-enforced (`src/sim/ownership.test.ts`), so F-001's "extensible write-back path" was already satisfied architecturally — only *which* roles are engineers and what each contributes remains undesigned.

**Leading direction.** Locked — build directly against this entry and [C-027-framing.md](candidates/C-027-framing.md): trait-mean fields per role per cell, first-order-toward-pressure-target update (same shape as L3's mortality-as-rate direction) at a turnover-derived rate, rendered via an extended `OccupantMesh`-style `InstancedMesh` sampling density (literal instance count) and trait-mean (bone-scale morph / Foxel rung-swap) deterministically per instance. Worked example: herbivore, with limb length ← terrain ruggedness, insulation ← temperature (plasticity), webbing ← inundation fraction (adaptation, threshold). §4 adds the trophic backbone: producers → herbivores/pollinators/dispersers → mesopredators → apex predators, decomposers closing the loop, engineers cross-cutting — mesopredator/apex/engineer roles stay unspecifiable until their own gaps (§4.1, §5) get a design pass; that pass is not this entry. Opens **Track A** — [BUILD_GUIDE §4.66](BUILD_GUIDE.md) (A1 Herbivore, first `populations` Process, D-007 clip gate applies) / [§4.67](BUILD_GUIDE.md) (A2 Seed disperser).

**Judge.** CI for determinism (identical seed/state → identical trait-mean trajectory and identical per-instance render sample), the no-fixed-K / no-tuned-rate guard, the envelope-bound/decline behavior, and the literal density→instance-count mapping. **Owner** for whether the resulting trait drift and morph/swap reads as earned, legible adaptation (**C-011**) rather than an arbitrary visual effect — the D-007 clip verdict on A1.

### C-028 — Sculpt toolbox vocabulary (sand-castle craft → Habitat causes)
**Status:** Open

**Question.** Which sand-castle craft tools are first-class Habitat sculpt **causes** (form / material under **C-006** / **A-005**), which reframe onto existing force or undo affordances, and which are banned as invented wet-sand physics, freeze-against-nature, or placeable life?

**Why now.** Owner kit-out (2026-08-02) named buckets, geometric molds, heavy shovel, watering can, trowels, carving needles, rake, blow straw, beachcomber stash, flags, miniature props, duplicator stamp, smoothing sponge, and freeze/dry spray — ahead of the terrain-tools wave [BUILD_GUIDE §4.54](BUILD_GUIDE.md) already queued. The metaphor is right (THESIS §2); a literal reading would reintroduce falling-sand carveability, freeze-against-the-tide, and painted biota that Locked entries already forbid. Recording the keep / reframe / ban map now keeps agents from inventing those, and names brush size as the first shippable cut.
...
### C-029 — Adaptive radiation as regional ecomorph divergence
**Status:** Open

**Question.** Once **C-027**'s per-role trait-mean field is live, should a population that spans genuinely isolated habitat patches under reduced competitive/predation pressure be allowed to diverge into visibly distinct, named regional morphs of the *same* role — the ecomorph pattern real adaptive radiation produces (Darwin's finch beaks, Caribbean *Anolis* ecomorphs, cichlid trophic morphs) — rather than staying one undifferentiated trait-mean across the whole preserve?

**Why now.** Owner design brief citing [adaptive radiation](https://en.wikipedia.org/wiki/Adaptive_radiation): ecological opportunity — released competition/predation, or colonization of a patchy/isolated habitat — relaxes stabilizing selection and lets a population diverge along different resource axes, producing a phenotype-environment correlation across forms with recent common ancestry. Filed now, alongside **C-027** Locking, because the mechanism turns out to need almost nothing new: the trait-mean field is already per-cell (spatial), **C-019** (island biogeography, Locked) already computes patch area/isolation, and **A2** (seed disperser, [BUILD_GUIDE §4.67](BUILD_GUIDE.md)) introduces the first dispersal-reach concept — a population's own connectivity is what turns "isolated" from an authored label into a simulated fact.

**Constraints.** Real adaptive radiation is literally speciation; Habitat's version cannot be — **W-003** (fixed curated species pool) and **N-003** (no species collection game) are Locked and forbid inventing new taxa. This entry is scoped to **regional ecomorph divergence within one fixed role**: the same distinction **C-027** already draws between frame (fixed species/role) and expression (fluid trait-mean), extended from one trait-mean per cell to clusters of trait-means that read as distinct forms across separated patches. No stochastic "radiation event" while **C-003** is Open — divergence must be a continuous, deterministic consequence of sustained isolation and released pressure, never a triggered spawn. No player-initiated radiation (**D-001** — nature is the protagonist; this must emerge from state, never be authored or buttoned). Inherits every **C-027** hard ban (no individual identity, no fixed-K, no free trait drift without survival cost, no bare threshold).

**Leading direction.** File as Open; implement nothing under it. If adopted, the only new mechanism is a second, space-keyed threshold-swap axis: instead of one global discrete ladder per trait, the ladder is selected per spatially-clustered population (a "which regional cluster is this cell part of" derived field, reusing **C-019**'s isolation math), gated on genuine patch isolation *and* measurably reduced competitive/predation pressure in at least one patch — not decoration, the actual trigger condition the science names. Framing: [C-029-framing.md](candidates/C-029-framing.md). Gated behind Locked **C-027** and **A1**+**A2** ([BUILD_GUIDE §4.66](BUILD_GUIDE.md)/[§4.67](BUILD_GUIDE.md)) shipping — nothing to diverge with one role and no dispersal concept yet. Not tip.
