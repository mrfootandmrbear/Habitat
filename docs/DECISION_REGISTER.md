...
**v2.0.18 — F-001 undeferred; C-027 Track A opened (Herbivore first).**

- Owner unblocked animal life. **F-001** (§15) moved Deferred → Current — the animal-work umbrella gate the rest of the docs already cite. Ecosystem-engineer write-back, F-001's literal scope, remains the hardest, still-undesigned piece (**E-005** owned-property path, flagged as a gap in C-027-framing.md §3.1) and is **not** unblocked by this entry.
- **C-027**'s slice-trigger updated: **L2**/**L3**/**L4** landed for the plant substrate; **L5** (blocked on **C-023**, Open, owner-judged) gates only the competition/predation portion of animal work (mesopredators, apex predators — C-027 §4), not trait expression for a non-competing role. Non-competitive, non-write-back roles may slice now.
- Opened **Track A (animal life)** in [BUILD_GUIDE.md](BUILD_GUIDE.md) §4.60–§4.61 and [CLOUD_AGENT_PIPELINE.md](CLOUD_AGENT_PIPELINE.md): **A1 Herbivore** (§4.60, tip) — C-027 §3.5's fully worked example, reusing existing slope / `factorTemperature` / `f_inundation` fields, no new resource field, no write-back; **A2 Seed disperser** (§4.61, next-but-one) — extends the same `populations` Process with a local coupling into L2's dispersal kernel.
- **C-027 stays Open.** The machine half (determinism, no-fixed-K, literal density→instance mapping) is CI-judged and self-promotable per DECISION_CONFORMANCE §3.0 once A1 ships; the legibility half stays owner-judged.
...
**Status:** Current *(undeferred 2026-08-03 — owner decision)*
...
*Why undeferred:* the owner lifted the animal-life block. Scope stays split, not wholesale: non-competitive, non-write-back roles (pollinators, seed dispersers, herbivores — **C-027**'s population trait-mean fields) are buildable now, starting with Herbivore ([BUILD_GUIDE §4.60](BUILD_GUIDE.md)). The literal content of this entry — ecosystem-engineer write-back — stays the hardest, still-undesigned piece: **E-005**'s owned-property/delta-inbox path (SIMULATION_MODEL §11) is not specced by C-027-framing.md §3.1, flagged there as a gap, not resolved by this status change. Mesopredator/apex-predator roles (food-web coupling, **ES-007**) stay gated behind **L5**/**C-023** per C-027 §4 — competition is a separate, later mechanism this undeferring does not unlock.
...
8m. **C-027** — animal trait expression as population fields (procedural morph + threshold swap); filed Open from an owner design chat, framed in [C-027-framing.md](candidates/C-027-framing.md). **F-001 undeferred 2026-08-03** — non-competitive roles (Herbivore, then Seed disperser) are now tip via Track A ([BUILD_GUIDE §4.60](BUILD_GUIDE.md)); mesopredator/apex-predator roles stay gated behind **L5**/**C-023**, and ecosystem-engineer write-back stays gated behind an unbuilt **E-005** design.
...
**Constraints.** No individual entity/identity store (**T-001**, **T-006**). No stochastic trait drift while **C-003** is Open — target must be a deterministic function of pressure fields. No fixed carrying-capacity constant (**ES-006**). No player-authored creature bodies — species identity stays resolved by **W-003**'s fixed pool; only trait expression is free-running (**N-003**, **N-005**, **D-001**). Every trait/pressure mapping needs a real-world referent (**C-011**, **N-004**). Must not smuggle in animal population competition/predation dynamics, which is **ES-007**/**C-023**/L5's separate, later scope.

**Leading direction.** File as Open; implement the machine half only where the Judge is CI (determinism, no-fixed-K, literal density mapping) — the owner half (legibility per **C-011**) stays a dossier until sat. Framing: [C-027-framing.md](candidates/C-027-framing.md) — trait-mean fields per role per cell, first-order-toward-pressure-target update (same shape as L3's mortality-as-rate direction), rendered via an extended `OccupantMesh`-style `InstancedMesh` sampling density (literal instance count) and trait-mean (morph/swap) deterministically per instance. Worked example: herbivore, with limb length ← terrain ruggedness, insulation ← temperature, webbing ← inundation fraction (threshold).

**Gate status (updated 2026-08-03).** **F-001** undeferred (§15); **L2**/**L3**/**L4** landed for the plant substrate. **L5** (guild competition) remains blocked on **C-023**, Open, owner-judged — but per C-027 §4, L5 is a precedent only the *competition/predation* portion of animal work needs (mesopredators, apex predators), not trait expression for a non-competing role. **Non-competitive, non-write-back roles may now slice: Herbivore first** ([BUILD_GUIDE §4.60](BUILD_GUIDE.md)), Seed disperser next ([§4.61](BUILD_GUIDE.md)). Ecosystem-engineer write-back (**E-005** path) and mesopredator/apex-predator roles stay off tip until, respectively, a write-back design lands and **L5**/**C-023** resolves.
