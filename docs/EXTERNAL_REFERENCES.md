...
|---|---|---|---|---|
| **Procedural/evolutionary animal simulators** — [biosim4](https://github.com/davidrmiller/biosim4), [Runxin0503/Artificial-Life](https://github.com/Runxin0503/Artificial-Life), [awesome-evolution-simulators](https://github.com/dietrich-stein/awesome-evolution-simulators) | Recalled from search results, 2026-08-05 — **not cloned or run**, same honesty class as the Game references row below | Nothing — logged as the negative reference. Confirms "evolution simulator" conventionally means individual-creature, NEAT/genetic-algorithm, stochastic-mutation architectures — the opposite of what **C-027** (Locked) already requires | Individual entity/identity store (**T-001**/**T-006** ban it outright); stochastic trait mutation while **C-003** is Open; per-creature neural-net "brains" as any part of Habitat's population model | Nothing — cited in [C-027-framing.md](candidates/C-027-framing.md) hard bans as the architecture explicitly *not* being built |
| **animalclub** — [minimapletinytools/animalclub](https://github.com/minimapletinytools/animalclub) | Recalled from search results, 2026-08-05 — **not cloned or run** | Confirms procedural 3D animal generation (genetics + mesh generation) is an active OSS category, structurally adjacent to the asset-generation half of C-027's render side | Adopting it as the asset tool — **Foxel** is already the decided toolchain for that role (2026-08-04, [VISUAL_UPGRADE_NOTE.md](VISUAL_UPGRADE_NOTE.md)); no reason to run two asset pipelines | Nothing |
...
| **Island colonization — tidal inundation / hydroperiod** | Same evidence §3 + NS-008 land 2026-07-31 | Envelope hydroperiod from elev vs MHW/MLW → herb Liebig `f_inundation`; foreshore stalls upland; ≠ salinity ≠ spray (**C-016**) | Collapse into `soil.salinity` / moisture; instantaneous tidal phase; new Process | `inundationComposition.ts`; `hsiComposition.ts`; probe `inundation-arrival`; `docs/slices/N8-composition.md` |
| **Island colonization — aspect light → arrival** | NATURAL_PROCESS_MATH §1.9 + NS-007 land 2026-07-31 | Open-sky `terrainInsolation` → herb Liebig `f_light` (normalized to horizontal); steep north stalls; Beer–Lambert stays succession-only (**C-007** / **C-011**) | Fold `understoryLight` into arrival HSI; hidden light multiplier; new Process | `lightComposition.ts`; `hsiComposition.ts`; probe `light-arrival`; `docs/slices/N7-composition.md` |
...
| **Island colonization — mid-marsh engineer** | Same evidence §2 mid-marsh + NS-009 land 2026-07-31 | Guild-local hydroperiod hump on same envelope as upland zero; mid-foreshore marsh ≫ herb; dry terrace herb ≫ marsh; physicalCover stack (**C-016** / **W-003**) | Fold hump into herb `habitat.suitability`; marsh painter; new Process | `marshHsiComposition.ts`; `factorInundationMarsh`; `veg.*.marsh`; probe `marsh-arrival`; `docs/slices/N9-composition.md` |
| **Island colonization — climate-capped woody shrub** | Same evidence §1 stage 3 + §2 climate-capped woody + NS-010 land 2026-07-31 | Warmer f_temp floor than herb; cover facilitation from herb fraction; warm herb-covered inland escalates; cold/mild/bare stay empty; physicalCover stack (**C-007** / **W-003** / NS-002) | Grass→shrub timers (ES-001); shrub painter; bird/guano Process | `shrubHsiComposition.ts`; `veg.*.shrub`; probe `shrub-arrival`; `docs/slices/N10-composition.md` |
| **Island colonization — cryptogam crust bootstrap** | Same evidence §1 stage 2 + §2 cryptogam crust + NS-011 land 2026-07-31 | Damp bare inland HSI (moisture × open canopy × salt × upland inundation); physicalCover raises infil (**C-007** / **W-003**) | Litter/OM Process; moss painter; nutrient/guano Process | `crustHsiComposition.ts`; `veg.*.crust`; probe `crust-arrival`; `docs/slices/N11-composition.md` |
...
| **three.js `Water.js` / `Water2.js`** (own addons, shipped in the project's own `node_modules/three`) | Source read in full, 2026-08-04 — both files, not summarized from docs | The mirror-camera + oblique-clip-plane technique (`Water.js`'s `onBeforeRender`) is legitimate prior art for **true geometric reflection** (terrain/vegetation actually appearing in the water), which `OceanMesh`/`WaterMesh` do not do today — both approximate reflection as a Fresnel-mixed flat sky-gradient lerp, so nothing ever reflects but the sky. Nothing landed yet; recorded as a study only | Adopting either class wholesale: `Water.js` needs a `waterNormals` **image asset** (this project procedurally generates every texture it uses — canvas-painted clouds, GLSL noise terrain detail — and ships none); `Water2.js` additionally defaults to loading two more normal-map JPGs via `TextureLoader` and stands up both a `Reflector` *and* a `Refractor` (two extra full-scene renders per frame) — a real cost against `QualityTier.ts`'s low-tier/iPad-Safari target (**GEO-002**-equivalent for render cost). Both are generic flat-plane water with no concept of this project's sim-driven elevation field, `seaLevel` force dial, or seabed skirt (`seabed.ts`) — bolting one on would sever the T-006 sim→depth-banding link that is the actual point of `OceanMesh`, not just swap a shader | Nothing yet — see recommendation below |
| **jbouny/ocean**, **madblade/waves-gerstner**, **davidllona/Threejs-water-shader** (GitHub, three.js ocean shaders) | Recalled from search results + repo descriptions, 2026-08-04 — **not cloned or run**, same honesty class as the Game references rows below | Confirms the two techniques generic three.js ocean demos converge on: **FFT/Tessendorf spectral waves** (jbouny/ocean) and **Gerstner/trochoidal vertex displacement** (madblade/waves-gerstner) for wave *shape*, plus procedural GLSL foam via smoothstep on wave-crest height (davidllona) as an alternative to a texture-based foam mask | Vertex-displaced waves of any kind as a drop-in: `OceanMesh`'s fragment shader comment states plainly that the plane's vertex Y is deliberately left undisplaced so the tuned shoreline `polygonOffset` z-fight fix (RENDER_NOTES.md) keeps holding — Gerstner/FFT displacement would reopen exactly that fix and has to re-solve it, not just add waves. FFT ocean specifically already sits in this doc's **Not helpful (skip)** table under "WebGPU mega-erosion demos" territory in spirit, if not letter — real-time spectral synthesis is a cost and complexity tier this stylized, top-down, sim-driven ocean has no use for | Nothing — this project's ocean is intentionally flat-plane; see recommendation below |
...
| **three.js `Water.js`** | `node_modules/three/examples/jsm/objects/Water.js` (ships with the project's own three.js dependency — no fetch needed) | Reference implementation of real geometric water reflection (mirror render target + oblique clip plane) — study *only* if a future round decides the sky-gradient Fresnel approximation in `OceanMesh`/`WaterMesh` genuinely needs to become a true scene reflection. Needs a normal-map image asset and a second scene render per frame; see study log for the specific costs before reaching for it. |
...
## Ocean/water rendering technique survey (2026-08-04)

**Why this exists.** Ocean rendering (`OceanMesh.ts`, `WaterMesh.ts`, `seabed.ts`) went through several gauntlet-loop rounds — near-black water, a muddy non-reflective plane, a hard "square tray" seam, boundary-cell extrusion spikes — resolved mostly by iterating on screenshots rather than by first checking what generic three.js ocean prior art already solves. This section is that check, done once, so the next round starts from it instead of re-deriving it. See the Study log above for the three specific entries (`Water.js`/`Water2.js`, `jbouny/ocean`, `madblade/waves-gerstner`, `davidllona/Threejs-water-shader`) this recommendation is built from.

**The honest finding: this project's ocean is not behind generic three.js ocean demos on technique — it is a different kind of ocean.** Every generic example above is a self-contained decorative plane with no relationship to any terrain data. `OceanMesh`'s actual job — band color and opacity by real depth read from the sim's own elevation field, agree exactly with `TerrainMesh`'s seabed skirt at every point outside the sim grid, and respond live to the `seaLevel` force dial — has no equivalent in any of them. There is nothing to "catch up" to; the hard parts already solved here (§3.2/§4.2 of [RENDER_SIM_INTERFACE.md](RENDER_SIM_INTERFACE.md)) are not the parts generic ocean shaders address at all.

**What is genuinely worth stealing, narrowly, if a future critique names it as the specific gap:**

1. **True geometric reflection**, if "water reflects sky/terrain" is ever scored as failing against the current Fresnel/sky-gradient approximation — study `Water.js`'s mirror-camera + oblique-clip-plane technique (already local, no fetch needed) rather than reinventing it. Cost-gate it: a second scene render per frame is real, and it should probably be tier-gated the same way SSAO/bloom already are in `QualityTier.ts`, not unconditional.
2. **Procedural (not texture-based) foam masking via domain-warped noise**, if foam is ever critiqued as too uniform — `OceanMesh`'s current foam is a depth-threshold band (`uFoamDepth`) already, not texture-based, so this is a refinement of an approach already in place, not a new one.

**What is explicitly not worth adopting, and why, so this doesn't get re-litigated:**

- **Any library's vertex-displaced waves (Gerstner or FFT).** `OceanMesh`'s fragment shader comment states the plane's vertex Y is deliberately left flat so the tuned shoreline `polygonOffset` z-fight fix (RENDER_NOTES.md, "Depth precision goes as far/near") keeps holding. Displacement reopens that fix; it would need to be re-solved, not just have waves added on top of it.
- **Swapping in `Water.js`/`Water2.js` wholesale.** Both are generic flat-plane water with zero awareness of a sim elevation field, `seaLevel`, or the seabed skirt. Adopting one as *the* ocean severs the T-006 sim→depth-banding link, which is the entire reason this project's ocean looks the way it does — this would be trading a bespoke, sim-correct system for a generic, sim-blind one, the opposite of an upgrade.
- **FFT/spectral ocean (jbouny/ocean-class).** Real-time cost and complexity this project's stylized, top-down camera and iPad-Safari-inclusive quality floor (`QualityTier.ts`) has no use for — same family of rejection as the WebGPU mega-erosion demos already banned below.
