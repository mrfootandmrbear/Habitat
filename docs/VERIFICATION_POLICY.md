...
| Basin fills to spill elevation, doesn't leak | **M** | Hand-derived Priority-Flood fixtures — single-pit plus a nested basin whose inner pit fills *above* the outer basin (`src/sim/fixtures/pitDem.ts`). Derived from Barnes, Lehman & Mulla 2014, not generated by RichDEM |
...
- Next scenarios: (none queued after Slice 14). Live: `paired-storm`, `berm-reroute`, `basin-fill`, `deep-time`, `baseflow-persist`, `regime-divergence`, `limiting-shift`, `burn-recover`, `succession-diverge`, `drydown-feedback`, `disturbance-recovery`, `arrival-earned`, `living-hollow`, `scenario-window`.
