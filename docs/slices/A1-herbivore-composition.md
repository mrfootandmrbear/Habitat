...
**Dev-only demo buttons.** The first playtest draft used raw DevTools console commands to stand in for A1's missing arrival tool; owner feedback after trying it was that this was too complicated. Replaced with two buttons in the **Full** controls panel (`src/ui/controls.ts` / `src/main.ts`, gated `import.meta.env.DEV`, never present in a shipped build): **Debug: seed herbivore herd** (instant founder population + forage, no waiting) and **Debug: fast-forward ~1 month** (~31 sim-days via direct `stepEvent()` calls, since real-time playback at the fastest rate measures too slow on typical hardware to cross even one seasonal band inside a playtest window). Both are explicitly labeled as debug stand-ins, not a second, undocumented arrival mechanism — A2 still owns building the real one.
