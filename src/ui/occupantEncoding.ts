...
/** Sand-binder mats — sandy khaki so crest cover reads apart from strand olive (C-009). */
...
/** Binder mat tint — sandy khaki vs strand olive (C-009 Tier-P). */
export function binderBiomassRgb(
  biomass: number,
  biomassMax: number,
): OccupantRgb {
  const t = Math.min(1, Math.max(0, biomass / Math.max(biomassMax, 1e-6)));
  ...
  return [
    ...
    lerp(BARE[2], BINDER[2], u),
  ];
}

/** Marsh turf tint — cool teal vs strand olive (C-016 Tier-P). */
export function marshBiomassRgb(
  biomass: number,
  biomassMax: number,
): OccupantRgb {
  const t = Math.min(1, Math.max(0, biomass / Math.max(biomassMax, 1e-6)));
...
export function binderOccupantEncodingDelta(
  biomass: number,
  strandMax: number,
  binderMax: number,
): number {
...
export function marshOccupantEncodingDelta(
  biomass: number,
  binderMax: number,
  marshMax: number,
): number {
